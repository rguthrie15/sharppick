...
async function publishToLeaderboard(){
  ...
  try {
    ...
    const sharpRating = s90.rating != null ? Number(s90.rating.toFixed(1)) :
                        (w90+l90 > 0 ? Number(Math.max(0,Math.min(100,(w90/(w90+l90)*100*0.7))).toFixed(1)) : 0);
    ...
    if (resp.ok) {
      markSupaOk();
      console.log('[SharpPick] Leaderboard rating published:', sharpRating);
    } else {
      const txt = await resp.text().catch(()=>'');
      console.warn('[SharpPick] publishToLeaderboard failed:', resp.status, txt);
    }
  } catch(e) {
...
async function _pushRatingsToSupabase(){
  ...
  // Try every possible token source in priority order
  const token =
    (typeof authSession !== 'undefined' && authSession?.access_token) ||
    (() => { try { return localStorage.getItem('sb_token'); } catch { return null; } })() ||
    (() => {
      // SUPA_HDR is updated on login: { Authorization: 'Bearer <token>' }
      try {
    ...
    (() => {
      // Supabase JS SDK stores session under sb-<project>-auth-token
      try {
  ...
  try {
    ...
    const sharpRating = Math.max(0, Math.min(100,
    ...
    const row = {
      ...
      is_provisional:      picks90count < 18,
      provisional_reason:  picks90count < 18 ? `${picks90count} settled picks (need 18)` : null,
      parlay_roi_90:       0,
      calculated_at:       new Date().toISOString(),
      calculated_for_date: new Date().toISOString().slice(0,10),
    };
    ...
    if (resp.ok) {
      console.log('[SharpPick] ✅ Ratings pushed to user_ratings');
      return true;
    }
    const errText = await resp.text().catch(()=>'');
    console.warn('[SharpPick] user_ratings upsert HTTP', resp.status, ':', errText);

    // Fallback: call an RPC if the table has a trigger or the view isn't directly writable
    try {
      await sbRpc('recalculate_user_rating', { p_user_id: currentUser.id });
      console.log('[SharpPick] recalculate_user_rating RPC triggered');
      return true;
    } catch(rpcErr) {
      console.warn('[SharpPick] RPC fallback failed:', rpcErr?.message);
    }
    return false;
...
async function renderLeaderboardPro(){
  ...
  const header = `
    <div class="lb-header">
      ...
      ${currentUser?.id === '0071fd52-32a2-4c0a-940b-97e8eea7885a' ? `<button class="lb-refresh" style="background:rgba(0,229,255,.08);border-color:rgba(0,229,255,.2);color:#00e5ff;margin-left:6px" onclick="(async()=>{const btn=this;btn.textContent='\u23f3 Settling...';btn.disabled=true;try{await fetchAndResettleHistoricalPicks();btn.textContent='\u23f3 Syncing...';await syncPicksToServerForced();btn.textContent='\u23f3 Pushing rating...';await new Promise(r=>setTimeout(r,1500));const ok=await _pushRatingsToSupabase();btn.textContent='\u23f3 Refreshing...';await new Promise(r=>setTimeout(r,2000));await renderLeaderboardPro();btn.textContent=ok?'\u2713 Done':'\u26a0\ufe0f Check console';}catch(e){btn.textContent='\u274c Error';console.error('RECALCULATE failed:',e);}setTimeout(()=>{btn.textContent='\u26a1 RECALCULATE';btn.disabled=false;},3000);})()">\u26a1 RECALCULATE</button>` : ''}
    </div>
  `;
...
function buildPickHTML(g){
  ...
  if(g.odds.spread){
    ...
    html+=`<div class="pick-row">
      <button class="pick-btn ${pA?'picked-spread':''} ${pA?.result||''} ${fin}" data-gid="${g.id}" data-type="spread" data-side="${g.away.name}" data-desc="${g.away.name} ${awayLine}" data-line="${awayLine}" data-ishome="0" data-game="${g.away.name} ${g.away.score} @ ${g.home.name} ${g.home.score}" onclick="pickFromBtn(this,event)"${lockTip}>${g.away.name}<br><span style="color:var(--gold);font-size:11px">${awayLine}</span></button>
      <button class="pick-btn ${pH?'picked-spread':''} ${pH?.result||''} ${fin}" data-gid="${g.id}" data-type="spread" data-side="${g.home.name}" data-desc="${g.home.name} ${homeLine}" data-line="${homeLine}" data-ishome="1" data-game="${g.away.name} ${g.away.score} @ ${g.home.name} ${g.home.score}" onclick="pickFromBtn(this,event)"${lockTip}>${g.home.name}<br><span style="color:var(--gold);font-size:11px">${homeLine}</span></button>
    </div>`;
...
function makePick(gameId,type,side,description,gameStr,extraData){
  ...
  picks.push({
    gameId, type, side, description, gameStr,
...
function checkPickResults(){
  ...
  picks.forEach(pick=>{
    ...
    if(pick.type==='spread'){
      ...
      // Guard: if |line| > 10 it is a MONEYLINE number (e.g. -218), not a point spread.
      // In that case settle as a straight moneyline (did team win outright?).
      const parts = (pick.description||'').trim().split(/\s+/);
      const rawLine = parseFloat(parts[parts.length - 1]);
      ...
      if(isNaN(rawLine) || Math.abs(rawLine) >= 50) {
        // Moneyline settlement — team simply needs to win outright
...
async function fetchAndResettleHistoricalPicks() {
...
function resettleAllPicks(){
  ...
  if (resetCount > 0) {
    ...
    // Force immediate server sync so leaderboard reflects corrected results
    setTimeout(() => { try { syncPicksToServerForced(); } catch{} }, 500);
  }
  return resetCount;
}
...
function openPanel(){
  ...
  // Async: fetch historical scores for any picks whose games are gone from live feed
...
function openSharpRatingExplainer(){
  ...
  const pillars = [
    { icon:'🎯', label:'WIN RATE', color:'#2ed573', desc:'Accuracy weighted by volume. Consistent winners beat lucky streaks.' },
  ...
  const tiers = [
    { icon:'🔥', name:'Elite',  range:'85–100', color:'#ff6b6b', req:'100+ picks req.' },
    ...
    { icon:'🥉', name:'Rookie', range:'0–39',   color:'#8a9ba8', req:'' },
  ];
  ...
  // Full-screen overlay — the INNER sheet scrolls, not the overlay
  ...
  overlay.innerHTML = `
    <div id="sharpExplainerSheet" style="
      width:min(520px,100%);
      height:88vh;
      background:#090e16;
    ...
    " onclick="event.stopPropagation()">
      ...
      ">
        <!-- Drag handle -->
        <div style="position:absolute;top:8px;left:50%;transform:translateX(-50%);width:36px;height:4px;background:rgba(255,255,255,.12);border-radius:999px"></div>
        ...
        <!-- ✕ always reachable at top right -->
        <button onclick="(${closeExplainer.toString()})()" style="
          flex-shrink:0;
          margin-top:6px;
      ...
      ">
        <div style="font-size:13px;color:rgba(255,255,255,.45);line-height:1.65;margin-bottom:22px">
          A skill-based score from <strong style="color:rgba(255,255,255,.7)">0–100</strong> measuring your betting performance over the last 90 days. No real money — just picks against real odds.
        </div>
...
async function syncPicksToServerForced(){
  // Bypasses throttle AND the Netlify function.
  // Writes settled pick results directly to user_picks via Supabase REST.
  // The Netlify function likely does INSERT-on-conflict-ignore, which skips
  // already-existing rows and never updates a result from 'lost' to 'won'.
  // Direct PATCH ensures corrected results actually reach the server.
  ...
  if (!token) {
    // No user token — fall back to normal Netlify sync
    lastSyncAt = 0;
  ...
  console.log(`[SharpPick] Direct-syncing ${settled.length} settled picks to Supabase...`);

  let updated = 0;
  let failed  = 0;

  for (const p of settled) {
    const syncId = p._syncId || p.id;
    if (!syncId) continue;

    const res = normalizeResult(p.result);
    try {
      // PATCH by id — updates only result + settled_at on the existing row
      const patchResp = await fetch(
        `${SUPA_REST}/user_picks?id=eq.${encodeURIComponent(syncId)}&user_id=eq.${currentUser.id}`,
        {
          method: 'PATCH',
          headers: {
            'apikey':        SUPA_KEY,
          ...
          body: JSON.stringify({
            result:     res,
            settled_at: Number(p.settledAt) || Date.now(),
            game_str:   p.gameStr   || undefined,
            final_score: p.finalScore || undefined,
          })
        }
      );
      ...
      if (patchResp.ok) {
        ...
        // Row might not exist yet — try a full upsert
        const row = pickToRow(p);
        if (!row) continue;
        const upsertResp = await fetch(`${SUPA_REST}/user_picks`, {
          method: 'POST',
          headers: {
            'apikey':        SUPA_KEY,
            'Authorization': `Bearer ${token}`,
        ...
        if (upsertResp.ok) { updated++; }
        else { failed++; }
      }
    } catch(e) {
      failed++;
      console.warn('[SharpPick] Direct pick sync error:', e?.message);
    }
  }

  console.log(`[SharpPick] Direct sync complete: ${updated} updated, ${failed} failed`);

  // Also run the normal Netlify sync for any pending/new picks
  lastSyncAt = 0;
  syncInProgress = false;
  try { await syncPicksToServer(); } catch {}
}
...
async function syncPicksFromServer(showIndicator = false) {
  ...
  try {
    ...
    serverPicks.forEach(sp => {
      ...
      if (localIdx === -1) {
        // Only add server pick locally if we recognise it by _syncId
        // (it was made on this device and is returning from server),
        // OR if it is still pending (could have been made on another device/browser).
        // Do NOT pull in old settled server picks that have no local counterpart —
        // those are pre-bug records that inflate the local record with wrong results.
        const hasSyncId = sp._syncId && picks.some(lp => lp._syncId === sp._syncId);
        if (hasSyncId || normalizeResult(sp.result) === 'pending') {
          picks.push(sp);
          merged = true;
        }
        return;
...
function openProfile(entry){
  ...
  overlay.innerHTML = `<div class="profile-sheet">
    ...
    ${!isMe && currentUser ? `<button class="profile-challenge-btn" onclick="startBattle('${entry.id}','${entry.name}');_closeProfileOverlay();setMode('battles')">⚔️ CHALLENGE ${entry.name.toUpperCase()} TO A BATTLE</button>` : ''}
