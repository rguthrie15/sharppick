...
function checkPickResults(){
  ...
  picks.forEach(pick=>{
    ...
    if(pick.type==='spread'){
      ...
      if(isNaN(rawLine) || Math.abs(rawLine) > 10) {
...
function resettleAllPicks(){
  // Force re-evaluation: reset any pick that was incorrectly settled
  // by clearing result on picks whose game is final and re-running settlement.
  // This corrects historical wrong settlements (e.g. moneyline mis-read as spread line).
  let resetCount = 0;
  picks.forEach(pick => {
    if(pick.result === 'pending') return;
    const g = allGames.find(x => x.id === pick.gameId);
    if(!g || !g.isFinal) return;
    // Re-open for re-settlement
    pick.result = 'pending';
    delete pick.finalScore;
    delete pick.settledAt;
    resetCount++;
  });
  if(resetCount > 0){
    checkPickResults();
    ...
    updateRecordUI();
    updateBankrollUI();
    console.log(`[SharpPick] Re-settled ${resetCount} picks`);
  }
  return resetCount;
}

function openPanel(){checkPickResults();checkPropPickResults();checkParlayResults();resettleAllPicks();renderPicksPanel();updateRecordUI();document.getElementById('picksPanel').classList.add('open');document.getElementById('panelOverlay').classList.add('open');}
