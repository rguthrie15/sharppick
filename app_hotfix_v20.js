...
async function renderLeaderboardPro(){
  ...
  const header = `
    <div class="lb-header">
      ...
      ${currentUser?.id === '0071fd52-32a2-4c0a-940b-97e8eea7885a' ? `<button class="lb-refresh" style="background:rgba(0,229,255,.08);border-color:rgba(0,229,255,.2);color:#00e5ff;margin-left:6px" onclick="(async()=>{this.textContent='⏳ Recalculating...';this.disabled=true;await fetchAndResettleHistoricalPicks();await new Promise(r=>setTimeout(r,3000));await renderLeaderboardPro();this.textContent='✓ Done';setTimeout(()=>{this.textContent='⚡ RECALCULATE';this.disabled=false;},2000);})()">⚡ RECALCULATE</button>` : ''}
    </div>
  `;
