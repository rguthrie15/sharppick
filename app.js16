...
async function fetchLeaderboard(){
  try{
    ...
    // Always do a safe two-step fetch:
    // 1) user_ratings (no relationship embeds — avoids PostgREST 400s)
    // 2) profiles display names (merged client-side)
    let rows = null;

    // Prefer supabase-js if present, but still no embeds
    if(typeof window !== 'undefined' && window.supabase && window.supabase?.from){
      const { data, error } = await window.supabase
        .from('user_ratings')
        .select('user_id,sharp_rating_90,win_rate_90,roi_90,units_90,picks_90,pending_90,all_time_singles,top_sport,top_sport_rating,cur_streak,avg_odds_last10,is_provisional,provisional_reason,recent_picks,bankroll')
      ...
      rows = await sbSelect(
        'user_ratings',
        `select=user_id,sharp_rating_90,win_rate_90,roi_90,units_90,picks_90,pending_90,all_time_singles,top_sport,top_sport_rating,cur_streak,avg_odds_last10,is_provisional,provisional_reason,recent_picks,bankroll&order=sharp_rating_90.desc&limit=${limit}`
      );
    }

    if(!rows || !rows.length) return [];

    // Pull display names from profiles (public table)
    let profileMap = {};
    try{
      const ids = rows.map(r=>r.user_id).filter(Boolean);
      const uniq = Array.from(new Set(ids));
      if(uniq.length){
        // PostgREST in.(...) requires comma-separated values
        const profRows = await sbSelect('profiles', `select=user_id,display_name&user_id=in.(${uniq.join(',')})&limit=${Math.min(1000, uniq.length)}`);
        profileMap = (profRows||[]).reduce((acc,p)=>{
          if(p?.user_id) acc[p.user_id] = (p.display_name||'').toString().trim();
          return acc;
        }, {});
      }
    }catch(e){
      // If profiles fetch fails, we still render leaderboard using fallbacks
      console.warn('profiles lookup failed', e?.message || e);
    }
    ...
    return rows.map(r=>{
      ...
      const profileName = (profileMap[r.user_id] || r.display_name || r.name || '').toString().trim();
      const name = profileName || (`user-${String(r.user_id||'').slice(0,6)}`);

      const picks90 = _num(r.picks_90, 0);
      const pending90 = _num(r.pending_90, 0);
      const settled90 = Math.max(0, picks90 - pending90);
      ...
      return {
        ...
        sharp,
        tier: _tierFromRating(sharp, picks90),
        ...
        // For Feed tab
        recentPicks: Array.isArray(r.recent_picks) ? r.recent_picks : (r.recent_picks ? (r.recent_picks||[]) : []),
        bankroll: _num(r.bankroll, 1000)
      };
    });
