...
async function syncPickemPickToServer(contestId, gameId, pickData){
  try{
    if(!currentUser?.id || !supaOnline) return;
    // Build a stable id so the upsert merge key works correctly
    const stableId = `${currentUser.id}_${contestId}_${gameId}`;
    const row = {
      id:         stableId,
      user_id:    currentUser.id,
...
async function syncPickemRecordToServer(){
  try{
    ...
    await sbUpsert('pickem_records', {
      id:          currentUser.id,  // stable merge key — one record per user
      user_id:     currentUser.id,
      user_name:   currentUser.name || '',
      wins:        rec.w || 0,
      losses:      rec.l || 0,
