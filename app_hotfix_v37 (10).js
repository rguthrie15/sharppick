...
// Set to true after RECALCULATE to skip RPCs and read fresh from user_ratings directly
let _lbSkipRpc = false;

async function fetchSharpLeaderboard(tab){
  const limit_n = 100;

  // Prefer RPCs (fast), but skip after RECALCULATE since RPCs may cache stale values.
  // RPCs are Supabase PostgreSQL functions that have their own formula — bypassing them
  // ensures the leaderboard reflects the rating we just pushed to user_ratings.
  if(!_lbSkipRpc){
    try{
      if(tab==='verified'){
  ...
  // Direct read from user_ratings — always up to date after RECALCULATE
  try{
