...
function fullRender(){
  ...
  // ── Guard: if user is mid-pick, defer full re-render by 1s to avoid wiping their panel ──
  if(Object.keys(pendingPickSel).length > 0){
    streamRenderTimer = setTimeout(()=>fullRender(), 1000);
    return;
  }
...
function renderScores(){
  ...
  // ── Guard: don't wipe the DOM if the user has an active prepick panel open ──
  // renderScores rebuilds innerHTML which destroys the panel and loses their pick selection.
  // Instead, skip this render cycle — patchScores() will keep scores current anyway.
  if(Object.keys(pendingPickSel).length > 0) return;
