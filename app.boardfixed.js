...
async function fetchLeaderboard(){
  try{
    ...
    const rows = await sbSelect(
      'user_ratings',
      `select=user_id,sharp_rating_90,win_rate_90,roi_90,units_90,picks_90,pending_90,all_time_singles,top_sport,top_sport_rating,cur_streak,avg_odds_last10,is_provisional,provisional_reason&order=sharp_rating_90.desc.nullslast&limit=${limit}`
    );
...
async function renderLeaderboardPro(){
  ...
  const rowHTML = (r, i)=>{
    const nm = _getDisplayNameForUserId(r.id);
