...
async function fetchLeaderboard(){
  try{
    ...
    // Progressive fallback: try specific columns → select=* with order → select=* no order → legacy table
    ...
    if(!rows){
      // Try 1: specific columns + order
      try{
        rows = await sbSelect('user_ratings', `select=${encodeURIComponent(selectCols)}&order=${orderCol}.desc&limit=${limit}`);
      }catch(e1){
        // Try 2: select=* with order (column list may be wrong)
        try{
          rows = await sbSelect('user_ratings', `select=*&order=${orderCol}.desc.nullslast&limit=${limit}`);
        }catch(e2){
          // Try 3: select=* no order (order column may not exist)
          try{
            rows = await sbSelect('user_ratings', `select=*&limit=${limit}`);
          }catch(e3){
            // Try 4: legacy 'leaderboard' table
            try{
              rows = await sbSelect('leaderboard', `select=*&order=pnl.desc.nullslast&limit=${limit}`);
            }catch(e4){
...
async function fetchSharpLeaderboard(tab){
  ...
  // Fallback 1: user_ratings with specific order (may 400 if column missing)
  try{
  ...
  // Fallback 2: user_ratings with no order (in case singles_overall_90 column doesn't exist)
  try{
