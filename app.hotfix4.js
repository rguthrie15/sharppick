...
function setMode(m){
  ...
  const dBtnsAll={...dBtns,calendar:'btnCalendar',news:'btnNews',bracket:'btnBracket',
  ...
  const desktopMoreViews=['calendar','news','bracket','feed','battles','trends'];
...
async function publishToLeaderboard(){
  // user_ratings is a server-computed table/view — client writes cause HTTP 400.
  // Leaderboard data is populated by Supabase RPCs (get_leaderboard_90, etc.)
  // and the sync-picks Netlify function. Do NOT upsert here.
  return;
}
...
function _tierFromRating(rating, picks){
  ...
  if(sr >= 85 && n >= 100) return 'Elite';
  if(sr >= 70) return 'Pro';
  if(sr >= 55) return 'Sharp';
  if(sr >= 40) return 'Solid';
  return 'Rookie';
}
...
let LB_SPORT = 'all';        // all | nba | nfl | mlb | nhl | etc.
...
function _filterBySport(rows){
  if(!LB_SPORT || LB_SPORT === 'all') return rows;
  const uid = currentUser?.id;
  return (rows||[]).filter(r=> String(r.topSport||'').toLowerCase() === LB_SPORT.toLowerCase() || r.id===uid);
}
...
async function renderLeaderboardPro(){
  ...
  const rows = _sortRows(_filterBySport(_filterRows(rowsRaw)));
    ...
    <div class="lb-header">
      <div>
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:10px;align-items:center">
          <span style="font-family:'DM Mono',monospace;font-size:9px;color:var(--dim);letter-spacing:1px">SPORT:</span>
          ${['all','NBA','NFL','MLB','NHL','NCAAB','NCAAF','MLS','EPL'].map(s=>`
            <button class="lb-tab ${LB_SPORT===(s==='all'?'all':s)?'on':''}"
              onclick="LB_SPORT='${s==='all'?'all':s}';renderLeaderboardPro()">${s==='all'?'All Sports':s}</button>`).join('')}
        </div>
      </div>
    ...
    <div class="lb-table-wrap">
      <table class="lb-table">
        <thead class="lb-head">
          <tr>
            <th>User</th>
            <th style="text-align:right" title="Sharp Rating — composite performance score">SR</th>
            <th style="text-align:right" title="Win % over last 90 days">Win%</th>
            <th style="text-align:right" title="Return on Investment">ROI</th>
            <th style="text-align:right" title="Units won/lost">Units</th>
            <th style="text-align:right" title="Settled picks (90 days)">Picks</th>
            <th style="text-align:right" title="Pending picks">⏳</th>
            <th style="text-align:right" title="All-time W-L-P record">Record</th>
          </tr>
        </thead>
        <tbody>
...
function renderHistoryView(){
  ...
  rows = rows.slice().sort((a,b)=>{
    if(histSortBy==='pnl'){
      const ap = Number(a?.pnl ?? a?.profit ?? 0) || 0;
      const bp = Number(b?.pnl ?? b?.profit ?? 0) || 0;
  ...
  const itemHTML = (p)=>{
    ...
    return `<div class="hist-item hist-${res}">
      ...
      ${p.gameStr?`<div class="hist-game">${p.gameStr}</div>`:''}
...
async function fetchSharpLeaderboard(tab){
  ...
  // Prefer RPCs (fast), but gracefully fall back if SQL/RPC isn't ready.
  try{
    if(tab==='verified'){
      const rows = await sbRpc('get_leaderboard_90',{limit_n});
      if(rows && rows.length) return rows;
    }else if(tab==='provisional' || tab==='specialists'){
      const rows = await sbRpc('get_leaderboard_90_provisional',{limit_n});
      if(rows && rows.length) return rows;
    }
  }catch(e){
    // fall through to view/table fallback
  }

  // Fallback 1: user_ratings ordered by sharp_rating_90
  try{
    const rows = await sbSelect('user_ratings', `select=*&order=sharp_rating_90.desc.nullslast&limit=${limit_n}`);
    return rows || [];
  }catch(e){}
...
function _sp_computeLocalUserStats(userId){
  ...
  // Simple rating proxy: blend win rate + ROI, clamp 0..100
  const rating = Math.max(0, Math.min(100, (aSingles90.winRate*0.7) + (Math.max(-100, Math.min(100, aSingles90.roi))*0.3)));
...
function pickemRecordFromHistory(){
  ...
  for(let i=0;i<localStorage.length;i++){
    const k=localStorage.key(i);
    if(!k||!k.startsWith('ls_contest_daily_')) continue;
    // Only count entries for the current user
    if(!k.endsWith('_'+(currentUser?.id||'anon'))) continue;
    try{
...
// ─── Contest History ─────────────────────────────────────────
const CONTEST_HISTORY_KEY = 'sharppick_contest_history';

function saveContestResult(contestId, result){
  try{
    const history = JSON.parse(localStorage.getItem(CONTEST_HISTORY_KEY)||'[]');
    if(!history.find(h=>h.id===contestId)){
      history.unshift({ id:contestId, ...result, savedAt: Date.now() });
      // Keep last 20
      localStorage.setItem(CONTEST_HISTORY_KEY, JSON.stringify(history.slice(0,20)));
    }
  }catch{}
}
...
function renderContestHistory(){
  ...
  let html = `<div style="display:flex;align-items:center;gap:10px;margin-bottom:14px">
    <button onclick="renderContestList()" style="background:none;border:none;color:var(--accent);font-family:'DM Mono',monospace;font-size:10px;cursor:pointer">← BACK</button>
    <div style="font-family:'DM Mono',monospace;font-size:11px;color:var(--muted)">PAST CONTESTS</div>
  </div>`;

  if(!history.length){
    html += `<div class="lb-empty" style="margin-top:20px">No past contests yet.<br><span style="font-size:11px;color:var(--dim)">Enter a contest to see your history here.</span></div>`;
  } else {
    html += history.map(h=>{
      const date = new Date(h.savedAt).toLocaleDateString([],{weekday:'short',month:'short',day:'numeric'});
      const pct = h.total>0 ? Math.round(h.correct/h.total*100) : 0;
      const color = pct>=60?'#2ed573':pct>=40?'var(--gold)':'#ff4757';
      return `<div style="background:var(--card);border:1px solid var(--border);border-radius:10px;padding:14px;margin-bottom:10px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
          <div style="font-family:'DM Mono',monospace;font-size:10px;color:var(--muted)">${date}</div>
          <div style="font-family:'DM Mono',monospace;font-size:9px;background:var(--bg);padding:3px 8px;border-radius:4px;color:var(--dim)">${h.id.startsWith('daily_')?'DAILY':'WEEKLY'}</div>
        </div>
        <div style="display:flex;gap:16px">
          <div><div style="font-size:20px;font-weight:900;color:${color}">${pct}%</div><div style="font-family:'DM Mono',monospace;font-size:9px;color:var(--dim)">WIN RATE</div></div>
          <div><div style="font-size:20px;font-weight:900">${h.correct||0}-${(h.total||0)-(h.correct||0)}</div><div style="font-family:'DM Mono',monospace;font-size:9px;color:var(--dim)">RECORD</div></div>
          ${h.rank ? `<div><div style="font-size:20px;font-weight:900;color:var(--gold)">#${h.rank}</div><div style="font-family:'DM Mono',monospace;font-size:9px;color:var(--dim)">RANK</div></div>` : ''}
        </div>
      </div>`;
    }).join('');
  }
...
async function renderContestList(){
  ...
  // Show "Past Results" link in contests header
  try{
    const hdr = document.querySelector('#contestsContent .section-hdr-row');
    if(hdr && !hdr.querySelector('.contest-history-btn')){
      const btn = document.createElement('button');
      btn.className='contest-history-btn';
      btn.style.cssText='background:none;border:none;color:var(--accent);font-family:"DM Mono",monospace;font-size:10px;cursor:pointer;padding:4px 8px;letter-spacing:.5px';
      btn.textContent='📋 HISTORY';
      btn.onclick = renderContestHistory;
      hdr.appendChild(btn);
    }
  }catch{}
...
async function renderContestDetail(contestId){
  ...
  <div style="margin-top:16px">
    ...
    <table class="contest-standings-table">
      <thead><tr><th>#</th><th>Name</th><th style="color:#2ed573">✓</th><th style="color:#ff4757">✗</th><th>Pending</th></tr></thead>
      <tbody>
...
function renderBestBetCard(){
  ...
  // Show Quick Pick card for new users with 0 settled picks
...
function renderQuickPickCard(el){
  // Find best pregame with odds for the Quick Pick
  const preGames = allGames.filter(g => g.isPre && (g.odds.spread || g.odds.total));
  if(!preGames.length){
    el.innerHTML = '';
    return;
...
function rebuyBankroll(){
  ...
  showConfirm(
    ...
    ()=>{
      ...
      // Sync rebuy count to server if possible
...
function updateBankrollUI(){
  ...
  pnl.textContent = `${pnlStr} ALL TIME${pendStr}${rebuyStr}`;
...
ctx.fillText(`${userName}  ·  ${dateStr}  ·  REC ${recStr}  ·  ${unitsStr}  ·  ${winStr}`, 24, H-18);

  // Footer mark (small) — new logo
  if(mark){
    ctx.save();
    ctx.globalAlpha = 0.7;
    ctx.drawImage(mark, W-56, H-38, 22, 22);
    ctx.restore();
  }
...
function openSharpRatingExplainer(){
  ...
  overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.85);z-index:9999;display:flex;align-items:flex-end;justify-content:center;padding:0;overflow-y:auto;-webkit-overflow-scrolling:touch;';
  overlay.onclick = e => { if(e.target === overlay) overlay.remove(); };

  const pillars = [
    { icon:'🎯', label:'ACCURACY', color:'#2ed573', desc:'Win rate weighted by odds quality. Beating long odds counts more than hitting favorites.' },
    { icon:'📈', label:'ROI', color:'#00e5ff', desc:'Profit per dollar risked. Measures whether you actually make money, not just win often.' },
    { icon:'📊', label:'CONSISTENCY', color:'#a855f7', desc:'Penalizes wild week-to-week swings. Steady profit beats boom/bust.' },
    { icon:'🔬', label:'SAMPLE SIZE', color:'#ffa502', desc:'More settled picks = higher confidence your rating is real, not luck.' },
  ];

  const tiers = [
    { icon:'🔥', name:'Elite', range:'85+', color:'#ff6b6b', req:'100+ picks' },
    { icon:'💎', name:'Pro', range:'70–84', color:'#a855f7', req:'' },
    { icon:'🥇', name:'Sharp', range:'55–69', color:'#00e5ff', req:'' },
    { icon:'🥈', name:'Solid', range:'40–54', color:'#ffa502', req:'' },
    { icon:'🥉', name:'Rookie', range:'0–39', color:'#636e72', req:'' },
  ];

  const pillarHTML = pillars.map(p => `
    <div style="display:flex;gap:12px;align-items:flex-start;padding:12px 14px;background:rgba(255,255,255,.02);border:1px solid rgba(255,255,255,.06);border-radius:12px">
      <div style="font-size:22px;flex-shrink:0;margin-top:2px">${p.icon}</div>
      <div>
        <div style="font-family:'DM Mono',monospace;font-size:10px;letter-spacing:2px;color:${p.color};font-weight:700">${p.label}</div>
        <div style="font-size:12px;color:rgba(255,255,255,.55);margin-top:4px;line-height:1.5">${p.desc}</div>
      </div>
    </div>`).join('');
  ...
  const tierHTML = tiers.map(t => `
    <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid rgba(255,255,255,.04)">
      ...
      <span style="font-weight:800;font-size:13px;color:${t.color};width:60px">${t.name}</span>
      <span style="font-family:'DM Mono',monospace;font-size:11px;color:rgba(255,255,255,.5);flex:1">${t.range}</span>
      ${t.req ? '<span style="font-family:\'DM Mono\',monospace;font-size:9px;color:rgba(255,255,255,.3)">'+t.req+'</span>' : ''}
    </div>`).join('');

  overlay.innerHTML = `
    <div style="width:min(520px,100%);max-height:92vh;overflow-y:auto;background:#0a0e14;border:1px solid rgba(0,229,255,.12);border-radius:20px 20px 0 0;padding:24px 20px 32px;box-shadow:0 -20px 60px rgba(0,0,0,.6);" onclick="event.stopPropagation()">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:20px">
        <div>
          <div style="font-family:'DM Mono',monospace;font-size:9px;letter-spacing:3px;color:#00e5ff;margin-bottom:6px">📐 RATING SYSTEM</div>
          <div style="font-family:Syne,sans-serif;font-size:22px;font-weight:800">What is a <span style="color:#00e5ff">Sharp Rating</span>?</div>
        </div>
        <button onclick="document.getElementById('sharpExplainerOverlay').remove()" style="background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.08);border-radius:10px;color:rgba(255,255,255,.5);font-size:16px;padding:6px 10px;cursor:pointer">✕</button>
      </div>

      <div style="font-size:13px;color:rgba(255,255,255,.5);line-height:1.6;margin-bottom:20px">
        A skill-based score from 0–100 measuring your betting performance over the last 90 days. No real money — just picks against real odds.
      </div>

      <div style="font-family:'DM Mono',monospace;font-size:9px;letter-spacing:2px;color:rgba(255,255,255,.3);margin-bottom:10px">THE FOUR PILLARS</div>
      <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:24px">
        ${pillarHTML}
      </div>

      <div style="padding:14px;background:rgba(255,255,255,.02);border:1px solid rgba(255,255,255,.06);border-radius:14px;margin-bottom:24px">
        <div style="font-family:'DM Mono',monospace;font-size:9px;letter-spacing:2px;color:rgba(255,255,255,.3);margin-bottom:12px">TIER BREAKDOWN</div>
        ${tierHTML}
      </div>

      <div style="display:flex;gap:10px;margin-bottom:20px">
        <div style="flex:1;padding:12px;background:rgba(0,229,255,.04);border:1px solid rgba(0,229,255,.10);border-radius:10px">
          <div style="font-family:'DM Mono',monospace;font-size:8px;letter-spacing:2px;color:#00e5ff;margin-bottom:4px">🔓 VERIFIED</div>
          <div style="font-size:11px;color:rgba(255,255,255,.5);line-height:1.5">20 settled singles to unlock. Verified ratings appear on the public leaderboard.</div>
        </div>
        <div style="flex:1;padding:12px;background:rgba(255,71,87,.04);border:1px solid rgba(255,71,87,.10);border-radius:10px">
          <div style="font-family:'DM Mono',monospace;font-size:8px;letter-spacing:2px;color:#ff4757;margin-bottom:4px">⚠️ DECAY</div>
          <div style="font-size:11px;color:rgba(255,255,255,.5);line-height:1.5">14+ days inactive = slight weekly rating decay. Stay active to keep your rank.</div>
        </div>
      </div>

      <div style="text-align:center;color:rgba(255,255,255,.25);font-family:'DM Mono',monospace;font-size:9px;letter-spacing:1px">
...
async function syncPicksToServer(){
  ...
  (() => { try { return localStorage.getItem('sb_token'); } catch { return null; } })();
    ...
    // Let other tabs/windows know picks changed (Realtime should also handle it)
...
async function syncPicksFromServer(showIndicator = false) {
  ...
  try {
    ...
    serverPicks.forEach(sp => {
      ...
      // Stamp syncId so future merges use ID-matching (faster + more accurate)
      if(sp._syncId && !lp._syncId){ picks[localIdx]._syncId = sp._syncId; merged = true; }
    ...
    // ── Phase 2: push any local picks the server doesn't have yet ──
    // This ensures picks made offline or on another device get uploaded
    const serverIds = new Set(serverPicks.map(sp=>sp._syncId).filter(Boolean));
    const serverGameKeys = new Set(serverPicks.map(sp=>`${sp.gameId}:${sp.type}:${sp.side}`));
    const missingLocally = picks.filter(lp=>{
      if(lp._syncId && serverIds.has(lp._syncId)) return false;
      return !serverGameKeys.has(`${lp.gameId}:${lp.type}:${lp.side}`);
    });
    if(missingLocally.length > 0){
      // Fire-and-forget upload of missing picks
      try{ syncPicksToServer(); }catch{}
    }

    if (!rows?.length && !missingLocally.length) {
...
function initPickSyncChannels(){
  try{
    ...
    // When the tab becomes visible again, re-sync once
...
function startPicksRealtime() {
  try {
    ...
    function doPoll() {
      ...
      syncPicksFromServer(false).then(() => {
        renderPicksPanel?.();
        updateRecordUI?.();
        renderScores?.();
...
function openProfile(entry){
  ...
  overlay.innerHTML = `<div class="profile-sheet">
    ...
    ${(()=>{
      const pendingTails = rp.filter(p=>normalizeResult(p.result)==='pending').slice(0,5);
      if(isMe || !currentUser || !pendingTails.length) return '';
      const rows = pendingTails.map(tp=>{
        const safe = JSON.stringify(tp).replace(/`/g,'\\`').replace(/\$/g,'\\$');
        return `<div style="display:flex;align-items:center;justify-content:space-between;background:var(--card);border:1px solid var(--border);border-radius:8px;padding:8px 12px;gap:10px">
          <div style="font-size:11px;color:var(--text);flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${tp.description||tp.type||'Pick'}</div>
          <button onclick="tailPick(${safe.replace(/'/g,"\'\'")});document.getElementById('profileOverlay').remove()"
            style="padding:5px 12px;background:rgba(0,229,255,.12);border:1px solid rgba(0,229,255,.3);border-radius:6px;color:#00e5ff;font-family:'DM Mono',monospace;font-size:10px;font-weight:700;cursor:pointer;white-space:nowrap">🔁 TAIL</button>
        </div>`;
      }).join('');
      return `<div style="padding:0 16px 12px">
        <div style="font-family:'DM Mono',monospace;font-size:9px;color:var(--muted);letter-spacing:1px;margin-bottom:8px">🔁 TAIL THEIR ACTIVE PICKS</div>
    ...
    ${isMe && typeof ACHIEVEMENTS !== 'undefined' ? (()=>{
      const unlocked = ACHIEVEMENTS.filter(a=>unlockedAchs.has(a.id));
      if(!unlocked.length) return '';
      return `<div style="padding:0 16px 12px">
        <div style="font-family:'DM Mono',monospace;font-size:9px;color:var(--muted);letter-spacing:1px;margin-bottom:8px">🏅 ACHIEVEMENTS (${unlocked.length}/${ACHIEVEMENTS.length})</div>
        <div style="display:flex;flex-wrap:wrap;gap:6px">
          ${unlocked.map(a=>`<div title="${a.desc}" style="display:inline-flex;align-items:center;gap:4px;padding:4px 10px;background:rgba(255,215,0,.08);border:1px solid rgba(255,215,0,.2);border-radius:999px;font-size:11px">
            <span>${a.icon}</span><span style="font-family:'DM Mono',monospace;font-size:9px;color:rgba(255,215,0,.9)">${a.name}</span>
          </div>`).join('')}
        </div>
      </div>`;
    })() : ''}
    ...
    <div style="padding:0 16px 8px">
      <button onclick="document.getElementById('profileOverlay').remove();openPlayerCard(${JSON.stringify(entry).replace(/"/g,'&quot;')})" style="width:100%;padding:12px;background:linear-gradient(135deg,rgba(0,229,255,.12),rgba(0,229,255,.04));border:1px solid rgba(0,229,255,.25);border-radius:8px;color:#00e5ff;font-family:'DM Mono',monospace;font-size:11px;font-weight:700;cursor:pointer;letter-spacing:1px">📤 SHARE PLAYER CARD</button>
    </div>
...
// ═══════════════════════════════════════════════════════
// TAIL A PICK — copy another user's pending pick to your slate
// ═══════════════════════════════════════════════════════
function tailPick(sourcePick){
  if(!currentUser){ showWinToast('⚠️ Sign in to tail picks'); return; }
  if(!sourcePick?.gameId && !sourcePick?.description){ showWinToast('⚠️ Cannot tail this pick'); return; }

  // Check if we already have this pick
  const already = picks.find(p=>
    p.gameId === sourcePick.gameId &&
    p.type === sourcePick.type &&
    p.side === sourcePick.side
  );
  if(already){
    showWinToast('✅ You already have this pick!');
    return;
  }

  // Clone the pick, reset to pending for current user
  const tailed = {
    gameId: sourcePick.gameId,
    type: sourcePick.type || 'spread',
    side: sourcePick.side || null,
    description: sourcePick.description || 'Tailed pick',
    gameStr: sourcePick.gameStr || '',
    result: 'pending',
    league: sourcePick.league || '',
    madeAt: Date.now(),
    wager: DEFAULT_WAGER,
    odds: sourcePick.odds || -110,
    confidence: sourcePick.confidence || 0,
    comment: sourcePick.comment ? `[Tailed] ${sourcePick.comment}` : '[Tailed]',
    isTailed: true,
  };
  ...
  renderPicksPanel();
  updateBankrollUI?.();
  showWinToast(`🔁 Tailed: ${tailed.description}`);
}
...
function renderWeeklyRecap(){
  // ── Guard 1: already dismissed this month ──
  const weekKey = `recap_dismissed_${new Date().toISOString().slice(0,7)}`;
  if(localStorage.getItem(weekKey)) return;
  // ── Guard 2: card already in DOM — fullRender() calls this on every poll ──
  if(document.getElementById('weeklyRecapCard')) return;
