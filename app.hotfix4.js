...
async function sbSelect(table, params=''){
  if(table==='leaderboard'){
    console.warn('[sbSelect] legacy table "leaderboard" requested; returning empty to avoid PostgREST errors.', params);
    return [];
  }
  try{
...
async function fetchLeaderboard(){
  // General leaderboard data, sourced from the user_ratings view.
  // NOTE: legacy `leaderboard` table has duplicate columns and can 400 in PostgREST.

  const sortMap = {
    sharp: 'sharp_rating_90',
    roi: 'roi_90',
    win: 'win_rate_90',
    units: 'units_90',
    picks: 'picks_90'
...
const name = dn || (uid ? `user-${uid.slice(0,6)}` : 'user');
      ...
      const units = Number(r.units_90 ?? 0);
      const picks = Number(r.picks_90 ?? 0);
      const pending = Number(r.pending_90 ?? 0);

      // record strings (optional)
      const record = r.all_time_singles || r.all_time_singles_record || '';
      ...
      return {
        ...
        picks,
        pending,
        settled: Math.max(0, picks - pending),
        record,
