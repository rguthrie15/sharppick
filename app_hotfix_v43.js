...
function renderPropsTable(g, statsData){
  ...
  propStats.forEach(statKey=>{
    ...
    statsData.teams.forEach(team=>{
      ...
      players.forEach(p=>{
        // NFL: skip irrelevant stat/position combos
        if(g.sport === 'football') {
...
async function loadDailyChallenge(){
  ...
  if(lg){
    try{
      ...
      if(isMLB) allPlayers = allPlayers.filter(p => !MLB_PITCHER_POSITIONS.includes((p.position||'').toUpperCase()));
      if(isNHL) allPlayers = allPlayers.filter(p => !['G','GK','GOALIE'].includes((p.position||'').toUpperCase()));
      if(isNFL){
      ...
      if(isMLB){ stat = 'Hits'; line = 1.5; }
      if(isNHL){ stat = 'Shots'; line = 2.5; }
      ...
      if(allPlayers.length){
        ...
        if(isNBA){ const pts = parseFloat(picked.stats?.pts)||0; if(pts>5){ stat='Points'; line=Math.round(pts*2-1)/2; } }
        else if(isMLB){ const avg=parseFloat(picked.stats?.avg)||0; line=avg>=0.280?1.5:0.5; }
        else if(isNHL){ const shots=parseFloat(picked.stats?.shots)||0; if(shots>1){ line=Math.round(shots*2-1)/2; } }
        else if(isNFL){ const pasYds=parseFloat(picked.stats?.passingYards)||0; const rusYds=parseFloat(picked.stats?.rushingYards)||0;
  ...
  if(!player){
    if(isMLB){ stat='Hits'; line=1.5; }
    else if(isNHL){ stat='Shots'; line=2.5; }
    else if(isNFL){ stat='Passing Yards'; line=229.5; }
    else { stat='Points'; line=22.5; }
