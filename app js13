        ...
        html += `<div class="prop-pick-row">
          ...
          <div class="prop-btns">
            <button class="prop-btn over ${existingPick?.side==='over'?'active':''} ${existingPick?.side==='over'?resultClass:''} ${fin||isSynthetic?'disabled':''}"
              data-gid="prop_${g.id}_${safeId}_${statKey}" data-type="prop" data-side="over" data-desc="${safeName} ${safeSL} O ${line}" data-game="${safeAway} vs ${safeHome}" data-actualgid="${g.id}" data-playerid="${safeId}" data-playername="${safeName}" data-statkey="${statKey}" data-statlabel="${safeSL}" data-line="${line}" onclick="pickFromBtn(this,event)">
            ...
            <button class="prop-btn under ${existingPick?.side==='under'?'active':''} ${existingPick?.side==='under'?resultClass:''} ${fin||isSynthetic?'disabled':''}"
              data-gid="prop_${g.id}_${safeId}_${statKey}" data-type="prop" data-side="under" data-desc="${safeName} ${safeSL} U ${line}" data-game="${safeAway} vs ${safeHome}" data-actualgid="${g.id}" data-playerid="${safeId}" data-playername="${safeName}" data-statkey="${statKey}" data-statlabel="${safeSL}" data-line="${line}" onclick="pickFromBtn(this,event)">
...
// DevTools helper (allows quick checks without exposing keys)
window.__SP = window.__SP || {};
window.__SP.testUserRatings = async () => sbSelect('user_ratings','select=user_id,sharp_rating_90,is_provisional,picks_90,pending_90&limit=10');
window.__SP.testProfiles = async () => sbSelect('profiles','select=user_id,display_name&limit=10');
...
async function fetchLeaderboard(){
  // Robust: never let a single PostgREST select hiccup blank the whole board.
  // 1) Try the full select.
  // 2) If it fails, retry with a minimal select that we know is safe.
  let rows = null;

  const fullSelect =
    'select=user_id,sharp_rating_90,win_rate_90,roi_90,units_90,picks_90,pending_90,' +
    'all_time_singles,top_sport,top_sport_rating,cur_streak,avg_odds_last10,' +
    'is_provisional,provisional_reason&order=sharp_rating_90.desc';
  ...
  try{
    rows = await sbSelect('user_ratings', fullSelect);
  }catch(e){
    console.warn('[LB] full select failed, retrying minimal:', e?.message||e);
  }

  if(!rows){
    try{
      rows = await sbSelect('user_ratings', minSelect);
    }catch(e){
      console.warn('[LB] minimal select failed:', e?.message||e);
      return [];
    }
  }
  ...
  // 2) Fetch profiles in bulk for names (best-effort; board still renders if this fails)
  const ids = rows.map(r=>r.user_id).filter(Boolean);
  let nameMap = {};
  try{
    // PostgREST IN syntax: user_id=in.(a,b,c)  (NO URL-encoding parens/commas)
    const inList = '(' + ids.join(',') + ')';
    const profs = await sbSelect(
      'profiles',
      'select=user_id,display_name&user_id=in.' + inList
    );
    (profs||[]).forEach(p=>{
      if(p?.user_id) nameMap[p.user_id] = (p.display_name||'').trim();
    });
  }catch(e){
    console.warn('[LB] profiles name fetch failed (non-fatal):', e?.message||e);
  }
  ...
  return rows.map(r=>{
    ...
    let tier = "Rookie";
    let tierIcon = "🥉";
    if(sharp >= 85 && (r.picks_90 ?? 0) >= 100){ tier="Elite"; tierIcon="🔥"; }
    else if(sharp >= 70){ tier="Pro"; tierIcon="💎"; }
    else if(sharp >= 55){ tier="Sharp"; tierIcon="🥇"; }
    else if(sharp >= 40){ tier="Solid"; tierIcon="🥈"; }

    // IMPORTANT: use is_provisional from DB; default to true if missing
    const provisional = (r.is_provisional ?? true) === true;

    // Verified unlock progress (20 picks)
    const goal = 20;
    const picks90 = Number(r.picks_90 ?? 0);
    const remaining = Math.max(0, goal - picks90);
    const unlockText = provisional ? `${picks90}/${goal} (${remaining} to Verified)` : `Verified`;

    const name =
      (nameMap[r.user_id] && nameMap[r.user_id].length ? nameMap[r.user_id] : '') ||
...
function pickFromBtn(btn, evt){
  ...
  meta: {
    actualGameId: btn.dataset.actualgid || null,
    playerId: btn.dataset.playerid || null,
    playerName: btn.dataset.playername || null,
    statKey: btn.dataset.statkey || null,
    statLabel: btn.dataset.statlabel || null,
    line: (btn.dataset.line!=null ? Number(btn.dataset.line) : null)
  }
};
...
// Place a player-prop pick (uses the same pre-pick wager flow as game picks)
function placePropPendingSelection(sel){
  try{
    if(!currentUser){ alert('Enter your name to make picks.'); return; }

    const actualGameId = sel?.meta?.actualGameId || null;
    const g = actualGameId ? allGames.find(x=>x.id===actualGameId) : null;
    if(g && !g.isPre) return; // lock once game starts

    const pickKey = sel.gid; // already composite: prop_<gameId>_<playerId>_<statKey>

    // Remove any existing prop pick for same key (toggle handled earlier by makePick path)
    picks = picks.filter(p => !(p.gameId===pickKey && p.type==='prop'));
    ...
    picks.push({
      ...
      playerId: sel?.meta?.playerId || null,
      statKey: sel?.meta?.statKey || null,
      line: (sel?.meta?.line!=null ? Number(sel.meta.line) : null),

      playerName: sel?.meta?.playerName || null,
      statLabel: sel?.meta?.statLabel || null,
    ...
    // Re-render props tab if open
    if(actualGameId && openGameId===actualGameId && g){
      try{ renderModalTab(g,'props'); }catch{}
    }
  }catch(e){
    console.warn('placePropPendingSelection failed:', e?.message || e);
  }
}
...
async function openProfileById(userId) {
  ...
  try {
    ...
    if(rows?.[0]) {
      const r = rows[0];
      openProfile({ id:r.user_id, name: (r.display_name || r.name || ''), w:r.w||0, l:r.l||0, p:r.p||0, total:r.total||0, bankroll:r.bankroll||1000, pnl:r.pnl||0, recentPicks:r.recent_picks||[] });
    }
  } catch(e) {}
}
