...
// CONFIG
// ═══════════════════════════════════════════════════════
const PROXIES      = ['', 'https://cors.isomorphic-git.org/', 'https://thingproxy.freeboard.io/fetch/', 'https://corsproxy.io/?url=', 'https://api.allorigins.win/raw?url='];
...
async function go(url,timeoutMs=8000){
  ...
  for(const pi of order){
    ...
    try{
      const u = pfx ? ((/corsproxy\.io|allorigins\.win/.test(pfx)) ? (pfx + encodeURIComponent(url)) : (pfx + url)) : url;
...
async function fetchLeaderboard(){
  try{
    // Pull Sharp Rating leaderboard data from user_ratings view/table
    const rows = await sbSelect(
      'user_ratings',
      'select=user_id,sharp_rating_90,win_rate_90,roi_90,units_90,picks_90,pending_90,all_time_singles,top_sport,top_sport_rating,cur_streak,avg_odds_last10,is_provisional,provisional_reason&order=sharp_rating_90.desc.nullslast'
    );
    ...
    return rows.map(r=>{
      ...
      let tier = "Rookie";
      if(sharp >= 85 && Number(r.picks_90 ?? 0) >= 100) tier = "Elite";
      ...
      return {
        ...
        winRate: Number(r.win_rate_90 ?? 0),
        roi: Number(r.roi_90 ?? 0),
        units: Number(r.units_90 ?? 0),

        picks: Number(r.picks_90 ?? 0),
        pending: Number(r.pending_90 ?? 0),

        record: r.all_time_singles ?? "0-0-0",

        topSport: r.top_sport ?? "",
        topSportRating: Number(r.top_sport_rating ?? 0),
    ...
    if(supaOnline!==false) console.warn("Leaderboard load failed", e?.message || e);
    return [];
  }
}
