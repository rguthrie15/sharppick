...
const MIN_PICKS    = 20;   // picks needed to be non-provisional (updated from 18)
...
function computeRatingRow(userId, picks) {
  ...
  s90.forEach(p => {
    ...
    byWeek[wk].risk += Math.max(0,+(p.wager||50));
  ...
  weeks.forEach(w => {
    ...
    const weekRoi = r > 0 ? (byWeek[w].pnl||0)/r : 0;
    if(weekRoi < -0.05) badWeeks++;
  });
  const rawConsistency = weeks.length > 0 ? Math.max(0, 100 - (badWeeks/weeks.length)*120) : 50;
  const consistencyScore = Math.max(0, Math.min(1000, rawConsistency * 10));
  // Volume multiplier: scales 0.7→1.0 as picks go from 5→50
  const volumeMult = Math.max(0.7, Math.min(1.0, 0.7 + (Math.min(s90.length, 50) / 50) * 0.3));
  // Final rating: 50% win rate + 30% ROI + 20% consistency, volume-adjusted
  const rawRating = (winRateScore * 0.5 + roiScore * 0.3 + consistencyScore * 0.2) * volumeMult;
  const sharpRating = Math.round(Math.max(0, Math.min(1000, rawRating)) * 10) / 10;
  // ─────────────────────────────────────────────────────────────
  ...
  // Build by-sport ratings for specialists leaderboard
  const bySport90 = {};
  Object.entries(byLeague).forEach(([lg, count]) => {
