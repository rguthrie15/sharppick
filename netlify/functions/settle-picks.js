...
async function settleAllPendingPicks() {
  ...
  for (const pick of spreadTotalPicks) {
    ...
    const ts = pick.made_at || Date.now();
    const baseMs = typeof ts === 'number' && ts < 2e12 ? ts : parseInt(ts) || Date.now();
    const dates = [
      ymd(baseMs - 86400000 * 2),
      ymd(baseMs - 86400000),
      ymd(baseMs),
      ymd(baseMs + 86400000),
    ];
  ...
  for (const userId of affectedUsers) {
    try {
      // on-pick-settled just needs a non-pending result in the payload
      await fetch('https://getsharppick.com/.netlify/functions/on-pick-settled', {
        ...
        body: JSON.stringify({
          ...
          table:  'user_picks',
          schema: 'public',
          record: { user_id: userId, result: 'won' }, // result just needs to be non-pending
        }),
      });
    } catch (e) {
...
exports.handler = async (event) => {
  // Auth check — accept both GET (from cron) and POST
