...
function schedulePoll(){
  ...
  pollTimer=setTimeout(async()=>{
    ...
    // Preserve cache as fallback — only wipe AFTER new data arrives to prevent skeleton flash
    const prevCache = cache[selDate];
    delete cache[selDate];
    try{sessionStorage.removeItem(SS_PREFIX+selDate);}catch(e){}
    const fetchOk = await fetchDate(selDate).then(()=>true).catch(()=>{ if(prevCache) cache[selDate]=prevCache; return false; });
    if(!fetchOk){ schedulePoll(); return; }
...
function startFastPoll(gameIds) {
  ...
  if(!gameIds?.length) return;
  // Use 13s interval — schedulePoll already fires every 5s during live games.
  // Staggering prevents both timers firing within the same second and double-patching the DOM.
  console.log(`⚡ Fast poll started for ${gameIds.length} live game(s) — 13s interval (staggered from 5s schedulePoll)`);
  ...
  }, 13000); // 13s — staggered from schedulePoll's 5s live cycle to prevent simultaneous DOM writes
}
