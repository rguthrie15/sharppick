...
async function fetchLeaderboard(){
  try{
    ...
    if(typeof window !== 'undefined' && window.supabase && window.supabase?.from){
      ...
      rows = await sbSelect(
        'user_ratings',
        `select=user_id,sharp_rating_90,win_rate_90,roi_90,units_90,picks_90,pending_90,all_time_singles,top_sport,top_sport_rating,cur_streak,avg_odds_last10,is_provisional,provisional_reason,profiles!user_ratings_user_id_fkey(display_name)&order=sharp_rating_90.desc&limit=${limit}`
      );
    }
