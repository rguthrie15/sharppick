...
function applyUser(){
  ...
  // These could never settle and would accumulate indefinitely. Remove them once on load.
...
async function syncPickemPickToServer(contestId, gameId, pickData){
  try{
    ...
    await sbUpsert('pickem_picks', row);
  } catch(e){
...
async function renderContestDetail(contestId){
  ...
  // Pull latest picks from server so leaderboard reflects all users across devices
...
async function contestPick(contestId,gameId,side){
  ...
  setTimeout(()=>{ try{ syncPickemRecordToServer(); }catch{} }, 1000);
}
...
function rebuyBankroll(){
  ...
  showConfirm(
    ...
    ()=>{
      ...
      try{ syncRebuyCountToServer(); }catch{}
    },
    'REBUY', false
  );
}
...
function pendingExposure(){
  return picks.filter(p=>normalizeResult(p.result)==='pending'&&p.wager).reduce((s,p)=>s+(p.wager||0),0);
}
