...
function schedulePoll(){
  ...
  pollTimer=setTimeout(async()=>{
    ...
    suppressFullRender = true;  // block fetchDate from triggering a fullRender flash during routine poll
...
function buildDateStrip(){
  ...
  if(strip.dataset.lastHtml !== newHtml){ strip.dataset.lastHtml=newHtml; strip.innerHTML=newHtml; }
...
function buildTabs(){
  ...
  if(tabsCont && tabsCont.dataset.lastHtml !== tabsHtml){ tabsCont.dataset.lastHtml=tabsHtml; tabsCont.innerHTML=tabsHtml; }
...
async function renderLeaderboardPro(opts={}){
  ...
  const lbHtml = header + table;
  // Only repaint leaderboard if content actually changed — avoids scroll reset and flash
  if(el.dataset.lastHtml !== lbHtml){ el.dataset.lastHtml = lbHtml; el.innerHTML = lbHtml; }
}
...
async function fetchAndResettleHistoricalPicks() {
  ...
  const now = Date.now();
  if (now - _resettleLastRun < 60_000) return 0;
  ...
  try{
    ...
    if(typeof BroadcastChannel !== 'undefined'){
      ...
      _picksBC.onmessage = async (ev)=>{
        ...
        renderScores?.();
        renderHistoryView?.();
        renderLeaderboardPro?.({silent:true});
      };
    }
    ...
    function doPoll() {
      ...
      syncPicksFromServer(false).then((merged) => {
        ...
        if(merged){
          ...
          updateRecordUI?.();
          renderScores?.();
          if(appMode==='leaderboard') renderLeaderboardPro?.({silent:true});
        }
...
function renderLivePickBar() {
  ...
  const barHtml = `<div style="font-family:'DM Mono',monospace;font-size:8px;letter-spacing:2px;color:#2ed573;padding:4px 12px;display:flex;align-items:center;gap:6px">
