...
function schedulePoll(){
  ...
  if(pi){
    ...
    // Guard writes — className changes trigger layout recalcs that interrupt ticker animation
...
function renderAnalyticsView(){
  ...
  ${(()=>{
    ...
    settled.forEach(p=>{
      ...
      if(p.result==='won') byMonth[mk].w++; else byMonth[mk].l++;
    });
    ...
    settled.forEach(p=>{
      ...
      if(p.result==='won') byHour[slot].w++; else byHour[slot].l++;
    });
    ...
    last30.forEach(p=>{
      if(p.wager){ wagered30+=p.wager;
        if(p.result==='won') profit30+=calcPayout(p.wager,p.odds||-110);
        else if(p.result==='lost') profit30-=p.wager;
      }
    });
      ...
      ${(()=>{
        ...
        const rows=buckets.map(b=>{
          ...
          const profit=ps.reduce((sum,p)=>{
            if(p.result==='won') return sum+calcPayout(p.wager,p.odds||-110);
            if(p.result==='lost') return sum-p.wager;
            return sum;
          },0);
...
function renderBestBetCard(){
  ...
  // Show Quick Pick card only for users who have never made any pick (including pending)
  const localPicks = Array.isArray(picks) ? picks : [];
  const hasPicks = localPicks.filter(p => p.type !== 'parlay').length > 0 ||
                   localPicks.filter(p => p.type === 'parlay').length > 0;
...
function updateBankrollUI(){
  ...
  const newPnl = `${pnlStr} ALL TIME${pendStr}${rebuyStr}`;
  ...
  overlay.innerHTML = `
    ...
    " onclick="event.stopPropagation()">
      ...
      ">
        ...
        <!-- ✕ fixed floating — always visible regardless of scroll on mobile -->
        <button id="sharpExplainerClose" onclick="(${closeExplainer.toString()})()" style="
          position:fixed;
          top:16px;
          right:16px;
      ...
      ">
        ...
        <!-- Bottom close button — always reachable on portrait mobile -->
        <button onclick="(${closeExplainer.toString()})()" style="
          display:block;width:100%;margin-top:8px;margin-bottom:16px;
