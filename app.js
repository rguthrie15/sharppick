...
async function fetchLeaderboard(){
  try{
    // 1) Pull ratings first (no joins) — avoids PostgREST FK/embed quirks breaking the whole board
    const rows = await sbSelect(
      'user_ratings',
      'select=user_id,sharp_rating_90,win_rate_90,roi_90,units_90,picks_90,pending_90,all_time_singles,top_sport,top_sport_rating,cur_streak,avg_odds_last10,is_provisional,provisional_reason&order=sharp_rating_90.desc'
    );
    ...
    try{
      ...
      const profs = await sbSelect(
        'profiles',
        'select=user_id,display_name&user_id=in.' + encodeURIComponent(inList)
      );
      (profs||[]).forEach(p=>{
        if(p?.user_id) nameMap[p.user_id] = (p.display_name||'').trim();
      });
    }catch(e){
      console.warn('profiles name fetch failed (non-fatal):', e?.message||e);
    }
