...
async function syncPicksFromServer(showIndicator = false) {
  ...
  try {
    ...
    serverPicks.forEach(sp => {
      ...
      if (localIdx === -1) {
        // On a fresh device pull ALL server picks (settled + pending)
        // On an existing device, only pull pending or picks we recognise by _syncId
        // to avoid inflating record with pre-bug duplicates
        const hasSyncId = sp._syncId && picks.some(lp => lp._syncId === sp._syncId);
        if (freshDevice || hasSyncId || normalizeResult(sp.result) === 'pending') {
          picks.push(sp);
          merged = true;
        }
        return;
