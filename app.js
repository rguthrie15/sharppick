...
async function generateAIBreakdown() {
  ...
  // Force a fresh sync from server before checking — RLS now requires
  // the user JWT which may not have been used on the initial page load sync.
  ...
  cardEl.innerHTML = `
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px">
      ...
      <div>
        <div style="font-family:'Syne',sans-serif;font-weight:800;font-size:13px;letter-spacing:.5px">AI PERFORMANCE BREAKDOWN</div>
        <div style="font-family:'DM Mono',monospace;font-size:9px;color:var(--muted);letter-spacing:1.5px">ANALYZING YOUR LAST ${Math.min(settled.length, 10)} PICKS…</div>
      </div>
    </div>
    <div style="display:flex;align-items:center;gap:8px;color:var(--muted);font-family:'DM Mono',monospace;font-size:11px">
  ...
  // Build pick summary for the last 10 settled picks (or however many exist)
  const last10 = settled.slice(-Math.min(settled.length, 10));
  const summary = last10.map(p => {
    const r = normalizeResult(p.result);
    return `${p.league || 'Unknown'} | ${p.type || 'spread'} | ${p.description || p.pick || 'Pick'} | ${r.toUpperCase()} | odds: ${p.odds || -110}`;
  }).join('\n');

  const stats = calcPickStats(picks);
  const pct = stats.decided > 0 ? Math.round(stats.w / stats.decided * 100) : 0;

  // Build by-sport breakdown
  const sports = {};
  settled.forEach(p => {
    const s = (p.league || 'Other').replace(/\s🏀|🏈|⚾|🏒|⚽/g,'').trim();
    if (!sports[s]) sports[s] = { w: 0, l: 0 };
    const _r = normalizeResult(p.result);
    if (_r === 'won') sports[s].w++; else if (_r === 'lost') sports[s].l++;
  });
  ...
  const prompt = `You are a sharp sports analytics coach reviewing a bettor's performance data. Be direct, specific, and actionable. Do NOT use generic advice. Respond in 3–4 short punchy sentences max.

Here is their data:
- Overall record: ${stats.w}W-${stats.l}L (${pct}% win rate)
- By sport: ${sportsSummary || 'not enough data per sport'}
- Last 10 picks:
${summary}

Give them: (1) one specific strength based on the data, (2) one specific weakness or pattern to watch, (3) one concrete actionable recommendation. Be like a sharp, no-nonsense coach. Use plain language, no bullet points, just 3-4 sentences.`;
    ...
    cardEl.innerHTML = `
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px">
        <div style="display:flex;align-items:center;gap:10px">
          ...
          <div>
            <div style="font-family:'Syne',sans-serif;font-weight:800;font-size:13px;letter-spacing:.5px">AI PERFORMANCE BREAKDOWN</div>
            <div style="font-family:'DM Mono',monospace;font-size:9px;color:var(--muted);letter-spacing:1.5px">BASED ON YOUR LAST 10 PICKS</div>
          </div>
        </div>
