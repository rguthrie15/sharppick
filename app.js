...
async function publishToLeaderboard(){
  ...
  try {
    ...
    // Unified Sharp Rating formula: 70% win rate + 30% ROI (same as _pushRatingsToSupabase)
    ...
    const sharpRating = s90.rating != null ? Number(s90.rating.toFixed(1)) :
                        Number(Math.max(0, Math.min(100,
                          winRate90_lb * 0.7 + Math.max(-100, Math.min(100, roi90)) * 0.3
                        )).toFixed(1));
...
async function renderContestDetail(contestId){
  ...
  try{
    lsKeysWithPrefix('ls_contest_'+contestId+'_').forEach(k=>{
      ...
      let ep={};
      try{ep=JSON.parse(localStorage.getItem(k)||'{}');}catch{return;}
...
// ── Debounced render wrappers ────────────────────────────────────
// Replace the originals with debounced versions so rapid sequential calls
// (e.g. sync + settlement + bankroll all firing within 50ms) collapse into one
...
let _initialSyncDone = false; // set true after first syncPicksFromServer completes
...
async function loadDailyChallenge(){
  // Pick the featured prop: find a pre-game NBA/NFL game
