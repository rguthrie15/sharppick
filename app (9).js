...
async function fetchLeaderboard(){
  try{
    ...
    // Prefer the Supabase client (carries auth token automatically)
    let rows = null;
    if(typeof supabase !== 'undefined' && supabase?.from){
      ...
      // Fallback: REST helper
      rows = await sbSelect(
        'user_ratings',
        `select=user_id,sharp_rating_90,win_rate_90,roi_90,units_90,picks_90,pending_90,all_time_singles,top_sport,top_sport_rating,cur_streak,avg_odds_last10,is_provisional,provisional_reason,profiles!user_ratings_user_id_fkey(display_name)&order=sharp_rating_90.desc.nullslast&limit=${limit}`
      );
    }
    ...
    const mapped = rows.map(r=>{
      ...
      // Override current user's record/picks so it reflects immediately
      let record = r.all_time_singles ?? '0-0-0';
      let picksDisplay = picks90;

      try{
        if(currentUser?.id && r.user_id === currentUser.id){
          const lr = _getLocalRecord();
          if(lr.total){
            record = `${lr.w}-${lr.l}-${lr.p}`;
            picksDisplay = Math.max(picksDisplay, lr.total);
          }
        }
      }catch{}
      ...
      return{
        id: r.user_id,
        name: (r?.profiles?.display_name || '').toString().trim(),

        sharp,
        tier: _tierFromRating(sharp, picks90),

        winRate: _num(r.win_rate_90, 0), // stored as percent (25 = 25%)
        roi: _num(r.roi_90, 0),          // stored as percent (-52.3 = -52.3%)
        units: _num(r.units_90, 0),

        picks: picksDisplay,
        pending: _num(r.pending_90, 0),
    ...
    // Ensure current user is always present even if RLS/joins cause them to be missing
    try{
      if(currentUser?.id && !mapped.some(x=>x.id===currentUser.id)){
        const my = await fetchMyRatings?.();
        if(my?.user_id){
          mapped.push({
            id: my.user_id,
            name: '',
            sharp: _num(my.sharp_rating_90 ?? my.singles_overall_90, 0),
            tier: _tierFromRating(_num(my.sharp_rating_90 ?? my.singles_overall_90, 0), _num(my.picks_90 ?? my.singles_picks_90, 0)),
            winRate: _num(my.win_rate_90, 0),
            roi: _num(my.roi_90, 0),
            units: _num(my.units_90, 0),
            picks: _num(my.picks_90 ?? my.singles_picks_90, 0),
            pending: _num(my.pending_90, 0),
            record: my.all_time_singles ?? '0-0-0',
            topSport: my.top_sport ?? '',
            sportRating: _num(my.top_sport_rating, 0),
            streak: my.cur_streak ?? '',
            avgOdds: my.avg_odds_last10 ?? '',
            provisional: !!my.is_provisional,
            provisionalReason: my.provisional_reason ?? ''
          });
        }
      }
    }catch{}
