...
async function _fetchTeamRosterImpl(sport, league, teamId, teamName, teamLogo){
  ...
  // For NFL, prioritize skill positions for props — exclude linemen/specialists
  const NFL_SKILL_POS = ['QB','RB','WR','TE','FB'];
  const NFL_EXCLUDE_POS = ['OL','OT','OG','C','DL','DE','DT','LB','CB','S','DB','K','P','LS','KR','PR'];
  let finalPlayers = rawPlayers;
  if(league === 'nfl') {
    const skillPlayers = rawPlayers.filter(p => NFL_SKILL_POS.includes(p.position));
    // Use skill players if we got enough, otherwise fall back to all non-linemen
    if(skillPlayers.length >= 6) {
      finalPlayers = skillPlayers;
    } else {
      finalPlayers = rawPlayers.filter(p => !NFL_EXCLUDE_POS.includes(p.position));
    }
  }
  ...
  return {
    ...
    players: finalPlayers.slice(0, 20),
  };
}
...
function getPlayerPropLine(player, statKey, cfg){
  ...
  const posDefaults = {
    ...
    baseball: {
      // Position-based realistic defaults — catchers/corner bats hit more HR/RBI
      C:  {h:0.5,hr:0.5,rbi:0.5},
      '1B':{h:1.5,hr:0.5,rbi:0.5},
      '2B':{h:1.5,hr:0.5,rbi:0.5},
      '3B':{h:1.5,hr:0.5,rbi:0.5},
      SS: {h:1.5,hr:0.5,rbi:0.5},
      LF: {h:1.5,hr:0.5,rbi:0.5},
      CF: {h:1.5,hr:0.5,rbi:0.5},
      RF: {h:1.5,hr:0.5,rbi:0.5},
      DH: {h:1.5,hr:0.5,rbi:0.5},
      OF: {h:1.5,hr:0.5,rbi:0.5},
      IF: {h:1.5,hr:0.5,rbi:0.5},
      _:  {h:0.5,hr:0.5,rbi:0.5},  // pitchers/unknown — shouldn't appear but safe fallback
    },
    hockey: {
...
function normalizeStats(raw, sport){
  ...
  const maps = {
    ...
    football: {
      // NOTE: 'yds' is ambiguous (ESPN reuses it across stat groups) — do not map it globally.
      // Use explicit full-name keys only so passing/rushing/receiving yards don't cross-contaminate.
      'att-cmp':'fg','td':'pasTD','int':'passInt',
      ...
      // ESPN boxscore stat group labels (lowercase, spaces stripped)
      'passingyards':'pasYds','rushingyards':'rushYds','receivingyards':'recYds',
      'passyards':'pasYds','rushyards':'rushYds','recyards':'recYds',
      'passingstats':'pasYds','rushingstats':'rushYds',
    },
    baseball: {
...
function renderPropsTable(g, statsData){
  ...
  propStats.forEach(statKey=>{
    ...
    statsData.teams.forEach(team=>{
      ...
      // For MLB pre-game, exclude pitchers (SP/RP/P/LHP/RHP/CL/SU) — they have no hitting stats
      const MLB_PITCHER_POS = ['SP','RP','P','LHP','RHP','CL','SU','PRP'];
      const isMLBGame = (g.sport === 'baseball');
      const filteredPlayers = isMLBGame && isPreGame
        ? team.players.filter(p => !MLB_PITCHER_POS.includes((p.position||'').toUpperCase()))
        : team.players;

      const players = isPreGame
        ? filteredPlayers.slice(0, isSynthetic ? 6 : 12)
        : filteredPlayers.filter(p=>p.active).slice(0, 12);
      ...
      players.forEach(p=>{
        // NFL: skip irrelevant stat/position combos (don't show pass yds for WRs, rec yds for QBs)
        if(g.sport === 'football') {
          const pos = (p.position||'').toUpperCase();
          if(statKey === 'pasYds' && !['QB'].includes(pos) && pos !== '') return;
          if(statKey === 'rushYds' && !['RB','FB','QB'].includes(pos) && pos !== '') return;
          if(statKey === 'recYds' && !['WR','TE','RB','FB'].includes(pos) && pos !== '') return;
        }
...
async function fetchSharpLeaderboard(tab){
  ...
  try{
    // Cache-bust with timestamp so we always get fresh data
    const cb = Date.now();
    if(tab==='verified'){
      const rows = await sbSelect('user_ratings',
        `select=*&singles_verified_90=eq.true&order=sharp_rating_90.desc.nullslast&limit=${limit_n}&_cb=${cb}`);
      if(rows && rows.length){
        console.log('[SharpPick] LB query returned ' + rows.length + ' rows, top SR=' + rows[0]?.sharp_rating_90 + ' user=' + rows[0]?.user_id?.slice(0,8));
        return rows;
      }
    } else {
      const rows = await sbSelect('user_ratings',
        `select=*&picks_90=gte.1&order=sharp_rating_90.desc.nullslast&limit=${limit_n}&_cb=${cb}`);
      if(rows && rows.length){
        console.log('[SharpPick] LB query returned ' + rows.length + ' rows, top SR=' + rows[0]?.sharp_rating_90 + ' user=' + rows[0]?.user_id?.slice(0,8));
        return rows;
      }
    }
  }catch(e){ console.warn('[SharpPick] LB query error:', e?.message); }
...
async function generateAIBreakdown() {
  const cardEl = document.getElementById('aiBreakdownCard');
  if (!cardEl) return;

  // Don't regenerate if already showing a result
  ...
  cardEl.innerHTML = `
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px">
      ...
      <div>
        <div style="font-family:'Syne',sans-serif;font-weight:800;font-size:13px;letter-spacing:.5px">AI PERFORMANCE BREAKDOWN</div>
        <div style="font-family:'DM Mono',monospace;font-size:9px;color:var(--muted);letter-spacing:1.5px">LOADING YOUR PICKS…</div>
      </div>
    </div>
    <div style="display:flex;align-items:center;gap:8px;color:var(--muted);font-family:'DM Mono',monospace;font-size:11px">
      <div class="stats-loading-ring"></div> SYNCING…
    </div>`;

  // Force a fresh sync from server before checking — RLS requires user JWT
  if (!currentUser?.isGuest) {
    try { await syncPicksFromServer(false); } catch(e) {
      console.warn('[generateAIBreakdown] sync failed:', e?.message);
    }
  }
  ...
  if (settled.length < 3) {
    cardEl.innerHTML = `
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px">
        <div style="font-size:20px">🤖</div>
        <div style="font-family:'Syne',sans-serif;font-weight:800;font-size:13px">AI PERFORMANCE BREAKDOWN</div>
      </div>
      <div style="font-family:'DM Mono',monospace;font-size:11px;color:var(--muted);line-height:1.6">
        You need at least 3 settled picks for an AI breakdown. Keep making picks and come back!
      </div>`;
    return;
  }

  const pickCount = Math.min(settled.length, 50);
  ...
  cardEl.innerHTML = `
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px">
      ...
      <div>
        <div style="font-family:'Syne',sans-serif;font-weight:800;font-size:13px;letter-spacing:.5px">AI PERFORMANCE BREAKDOWN</div>
        <div style="font-family:'DM Mono',monospace;font-size:9px;color:var(--muted);letter-spacing:1.5px">ANALYZING YOUR LAST ${pickCount} PICKS…</div>
      </div>
    </div>
    <div style="display:flex;align-items:center;gap:8px;color:var(--muted);font-family:'DM Mono',monospace;font-size:11px">
      <div class="stats-loading-ring"></div> THINKING…
    </div>`;

  // ── Data assembly ────────────────────────────────────────────────

  const last50 = settled.slice(-pickCount);

  // Overall record
  const stats = calcPickStats(picks);
  const pct = stats.decided > 0 ? Math.round(stats.w / stats.decided * 100) : 0;

  // Pick-by-pick summary (newest first)
  const summary = [...last50].reverse().map(p => {
    const r = normalizeResult(p.result);
    const date = p.madeAt ? new Date(p.madeAt).toLocaleDateString('en-US',{month:'short',day:'numeric'}) : '';
    return `${date} | ${(p.league||'Unknown').replace(/\s*[🏀🏈⚾🏒⚽🏐🎾]/gu,'').trim()} | ${p.type||'spread'} | ${p.description||p.side||'Pick'} | ${r.toUpperCase()} | odds:${p.odds||-110}`;
  }).join('\n');

  // By-sport breakdown
  const sports = {};
  last50.forEach(p => {
    const s = (p.league||'Other').replace(/\s*[🏀🏈⚾🏒⚽🏐🎾]/gu,'').trim();
    if (!sports[s]) sports[s] = { w:0, l:0 };
    const r = normalizeResult(p.result);
    if (r==='won') sports[s].w++; else if (r==='lost') sports[s].l++;
  });
  const sportsSummary = Object.entries(sports)
    .filter(([,v]) => v.w + v.l >= 2)
    .map(([s,v]) => `${s}: ${v.w}-${v.l} (${Math.round(v.w/(v.w+v.l)*100)}%)`)
    .join(', ') || 'not enough data per sport';
  ...
  last50.forEach(p => {
    ...
    const r = normalizeResult(p.result);
    if (r==='won') byType[t].w++; else if (r==='lost') byType[t].l++;
  });
  const typeSummary = Object.entries(byType)
    .filter(([,v]) => v.w + v.l >= 2)
    .map(([t,v]) => `${t}: ${v.w}-${v.l} (${Math.round(v.w/(v.w+v.l)*100)}%)`)
    .join(', ') || 'not enough data per type';

  // Favorite vs underdog
  const favs = last50.filter(p => Number(p.odds) <= -120);
  const dogs = last50.filter(p => Number(p.odds) >= 100);
  const picksEv = last50.filter(p => Number(p.odds) > -120 && Number(p.odds) < 100);
  const favW = favs.filter(p => normalizeResult(p.result)==='won').length;
  const dogW = dogs.filter(p => normalizeResult(p.result)==='won').length;
  const evW  = picksEv.filter(p => normalizeResult(p.result)==='won').length;
  const favRecord = favs.length  >= 2 ? `${favW}-${favs.length-favW} (${Math.round(favW/favs.length*100)}%)` : 'not enough data';
  const dogRecord = dogs.length  >= 2 ? `${dogW}-${dogs.length-dogW} (${Math.round(dogW/dogs.length*100)}%)` : 'not enough data';
  const evRecord  = picksEv.length >= 2 ? `${evW}-${picksEv.length-evW} (${Math.round(evW/picksEv.length*100)}%)` : 'not enough data';

  // Average odds
  const avgOdds = Math.round(last50.reduce((a,p) => a + Number(p.odds||-110), 0) / last50.length);

  // Day-of-week breakdown
  const byDay = {};
  last50.forEach(p => {
    if (!p.madeAt) return;
    const day = new Date(p.madeAt).toLocaleDateString('en-US',{weekday:'short'});
    if (!byDay[day]) byDay[day] = { w:0, l:0 };
    const r = normalizeResult(p.result);
    if (r==='won') byDay[day].w++; else if (r==='lost') byDay[day].l++;
  });
  ...
  // Recent momentum: last 10 vs prior picks
  const recent10 = last50.slice(-10);
  const prior = last50.slice(0, last50.length - 10);
  const r10W = recent10.filter(p => normalizeResult(p.result)==='won').length;
  const r10pct = recent10.length > 0 ? Math.round(r10W/recent10.length*100) : 0;
  const priorW = prior.filter(p => normalizeResult(p.result)==='won').length;
  const priorPct = prior.length > 0 ? Math.round(priorW/prior.length*100) : 0;
  const momentumNote = prior.length >= 5
    ? `Last 10: ${r10W}-${recent10.length-r10W} (${r10pct}%) vs prior ${prior.length} picks: ${priorW}-${prior.length-priorW} (${priorPct}%)`
    : `Last ${recent10.length} picks: ${r10W}-${recent10.length-r10W} (${r10pct}%)`;

  // ── Build prompt ─────────────────────────────────────────────────

  const prompt = `You are a sharp sports analytics coach reviewing a bettor's detailed performance data. Be direct, specific, and actionable. Reference actual numbers from their data. Do NOT give generic advice like "shop for lines" or "manage your bankroll" unless the data specifically supports it.

BETTOR STATS (last ${pickCount} settled picks):
- Overall: ${stats.w}W-${stats.l}L (${pct}% win rate)
- Momentum: ${momentumNote}
- By sport: ${sportsSummary}
- By bet type: ${typeSummary}
- Favorites (-120 or worse): ${favRecord}
- Underdogs (+100 or better): ${dogRecord}
- Near-even odds (-119 to +99): ${evRecord}
- Average odds taken: ${avgOdds}
- Day of week patterns: ${daySummary}

PICK LOG (newest first):
${summary}

Analyze this data and give them 4-5 punchy sentences covering: (1) their single clearest edge or strength with the specific number, (2) their single biggest leak or losing pattern with the specific number, (3) what their momentum trend says about their current form, (4) one concrete tactical adjustment they should make this week based on the data. Talk like a sharp no-nonsense coach. No bullet points. No generic advice. Every sentence must reference a specific stat from their data.`;

  try {
    const response = await fetch('/.netlify/functions/ai-breakdown', {
      method: 'POST',
    ...
    const text = data.text || 'Unable to generate breakdown right now.';
    ...
    cardEl.innerHTML = `
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px">
        <div style="display:flex;align-items:center;gap:10px">
          ...
          <div>
            <div style="font-family:'Syne',sans-serif;font-weight:800;font-size:13px;letter-spacing:.5px">AI PERFORMANCE BREAKDOWN</div>
            <div style="font-family:'DM Mono',monospace;font-size:9px;color:var(--muted);letter-spacing:1.5px">BASED ON YOUR LAST ${pickCount} PICKS</div>
          </div>
        </div>
        <button onclick="refreshAIBreakdown()" style="background:transparent;border:1px solid var(--border);border-radius:8px;padding:4px 10px;font-family:'DM Mono',monospace;font-size:9px;color:var(--muted);cursor:pointer;letter-spacing:1px">↻ REFRESH</button>
      </div>
      <div style="font-family:'DM Mono',monospace;font-size:12px;line-height:1.7;color:var(--text);border-left:2px solid var(--accent);padding-left:12px">${text}</div>
      <div style="margin-top:10px;font-family:'DM Mono',monospace;font-size:9px;color:var(--dim);letter-spacing:1px">POWERED BY CLAUDE AI</div>`;
    ...
    cardEl.innerHTML = `
      ...
      <div style="font-family:'DM Mono',monospace;font-size:11px;color:var(--muted)">Unable to load breakdown. Try refreshing.</div>
      <button onclick="refreshAIBreakdown()" style="margin-top:8px;background:transparent;border:1px solid var(--border);border-radius:8px;padding:6px 12px;font-family:'DM Mono',monospace;font-size:10px;color:var(--muted);cursor:pointer">↻ TRY AGAIN</button>`;
  }
}
function refreshAIBreakdown() {
  const cardEl = document.getElementById('aiBreakdownCard');
  if (cardEl) {
...
function renderAnalyticsView(){
  ...
  ${settled.length >= 10 ? `
  <div class="section-hdr-row" style="margin-top:16px">
    <div><div class="section-title">🤖 AI Breakdown</div><div class="section-sub">PERSONALIZED COACHING FROM YOUR DATA</div></div>
  </div>
  <div id="aiBreakdownCard" class="analysis-card" style="margin-bottom:8px;border:1px solid rgba(0,229,255,.2);background:rgba(0,229,255,.03);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:24px;text-align:center">
    <div style="font-size:28px;margin-bottom:10px">🤖</div>
    <div style="font-family:'Syne',sans-serif;font-weight:800;font-size:13px;letter-spacing:.5px;margin-bottom:6px">GET YOUR AI BREAKDOWN</div>
    <div style="font-family:'DM Mono',monospace;font-size:10px;color:var(--muted);margin-bottom:16px">Personalized coaching based on your last 10 picks</div>
    <button onclick="generateAIBreakdown()" style="background:var(--accent);color:#000;border:none;border-radius:10px;padding:10px 24px;font-family:'DM Mono',monospace;font-size:11px;font-weight:700;letter-spacing:1.5px;cursor:pointer">ANALYZE MY PICKS →</button>
  </div>` : ''}
...
function openPlayerCard(entry){
  ...
  if(isMe){
    ...
    // Pull rating from Supabase cache (user_ratings) — the local snapshot
    // returns null because ratings are server-computed, not client-computed.
    // fetchMyRatings() is async so we re-render the card once data arrives.
    sharp = e.sharp || 0;
    tier = 'Rookie';
    topSport = e.topSport || '';
    ...
    record = `${w}-${l}-${p}`;
    // Async: fetch real rating from Supabase and re-render card
    if(typeof fetchMyRatings === 'function'){
      fetchMyRatings().then(r => {
        if(!r) return;
        const sr = r.sharp_rating_90 ?? r.singles_overall_90 ?? 0;
        const verified = !!r.singles_verified_90;
        const tier2 = tierLabelFromRating(sr, verified) || _tierFromRating(sr, r.picks_90||0);
        const canvas2 = window._playerCardCanvas;
        if(canvas2 && sr){
          renderPlayerCardCanvas(canvas2.getContext('2d'), canvas2.width, canvas2.height, {
            ...
            tier: tier2,
            topSport: r.top_sport || '',
            topSportRating: r.top_sport_rating || 0,
            streak: r.cur_streak || '',
            roi: r.roi_90 || 0,
            units: r.units_90 || 0,
            picks90: r.picks_90 || picks90,
...
async function loadDailyChallenge(){
  ...
  if(!game) game = allGames.find(g=>g.isPre);
  if(!game) return null;

  const isMLB = game.leagueLabel.includes('MLB');
  const isNHL = game.leagueLabel.includes('NHL');
  const isNBA = game.leagueLabel.includes('NBA');
  const isNFL = game.leagueLabel.includes('NFL');

  // Stat map: human label → ESPN boxscore key, per sport
  const CHALLENGE_STAT_KEY = {
    // Basketball
    ...
    // Football
    'Passing Yards': 'passingYards', 'Rushing Yards': 'rushingYards', Receptions: 'receptions',
  };

  // Pitcher position codes to exclude from MLB challenges
  const MLB_PITCHER_POSITIONS = ['SP','RP','P','LHP','RHP','CL','SU'];

  const lg = LEAGUES.find(l=>l.league===game.league);
  let player = null, playerId = null;

  // Sport-specific default stat + line (used if avg stats unavailable)
  let stat = 'Points';
  let line = isNBA ? 22.5 : isNFL ? 55.5 : isMLB ? 1.5 : isNHL ? 2.5 : 22.5;
  ...
  if(lg){
    try{
      ...
      // ── MLB: exclude pitchers, pick hitters only ──────────────────
      if(isMLB){
        allPlayers = allPlayers.filter(p => {
          const pos = (p.position || p.pos || '').toUpperCase();
          return !MLB_PITCHER_POSITIONS.includes(pos);
        });
        // Use Hits as the default MLB stat — universally applicable to all hitters
        stat = 'Hits';
        line = 1.5;
      }

      // ── NHL: use skaters only (exclude goalies) ───────────────────
      if(isNHL){
        allPlayers = allPlayers.filter(p => {
          const pos = (p.position || p.pos || '').toUpperCase();
      ...
      // ── NFL: pick by position for relevant stats ──────────────────
      if(isNFL){
        const qbs = allPlayers.filter(p => (p.position||'').toUpperCase() === 'QB');
        const rbs = allPlayers.filter(p => ['RB','FB'].includes((p.position||'').toUpperCase()));
        const wrs = allPlayers.filter(p => ['WR','TE'].includes((p.position||'').toUpperCase()));
        // Prefer QBs for passing yards, otherwise use skill positions
        allPlayers = qbs.length ? qbs : [...rbs, ...wrs];
        stat = qbs.length ? 'Passing Yards' : rbs.length ? 'Rushing Yards' : 'Receptions';
        line = qbs.length ? 229.5 : rbs.length ? 54.5 : 4.5;
      }

      if(allPlayers.length){
        // Deterministic pick by date — stable for the whole day
        const seed = parseInt(todayKey().replace(/-/g,'')) % allPlayers.length;
        const picked = allPlayers[seed];
        player   = picked.name;
        playerId = picked.id || null;

        // Set line from season avg stats if available
        if(isNBA){
          const pts = parseFloat(picked.stats?.pts) || 0;
          if(pts > 5){ stat = 'Points'; line = Math.round(pts * 2 - 1) / 2; }
        } else if(isMLB){
          const avg = parseFloat(picked.stats?.avg) || 0;
          // Derive hits line from batting average × at-bats estimate
          stat = 'Hits';
          line = avg >= 0.280 ? 1.5 : 0.5;
        } else if(isNHL){
          const shots = parseFloat(picked.stats?.shots) || 0;
          if(shots > 1){ stat = 'Shots'; line = Math.round(shots * 2 - 1) / 2; }
        } else if(isNFL){
          const pasYds = parseFloat(picked.stats?.passingYards) || 0;
          const rusYds = parseFloat(picked.stats?.rushingYards) || 0;
          if(pasYds > 50){ stat = 'Passing Yards'; line = Math.round(pasYds / 10) * 10 - 0.5; }
          else if(rusYds > 10){ stat = 'Rushing Yards'; line = Math.round(rusYds / 5) * 5 - 0.5; }
        }
      }
    }catch(e){ console.warn('[DailyChallenge] roster fetch failed:', e?.message); }
  }

  // Fallback if roster fetch failed or no valid players found
  if(!player){
    ...
    player = `${game.away.abbr||game.away.name} vs ${game.home.abbr||game.home.name} — Top Performer`;
  }
...
async function syncPicksFromServer(showIndicator = false) {
  ...
  // Guest users have no JWT — they can't pass RLS on user_picks
  if (currentUser.isGuest) return false;

  try {
    if (showIndicator) showSyncIndicator('syncing');

    // user_picks is RLS-protected: must use the user's JWT, not the anon key.
    // sbSelect uses SUPA_HDR which starts as anon on page load and only gets
    // the user token after login completes — so we always read the token explicitly here.
    const userToken = _getUserToken();
    if (!userToken) {
      console.warn('[syncPicksFromServer] No user token — skipping sync');
    ...
    const resp = await fetch(
      `${SUPA_REST}/user_picks?user_id=eq.${currentUser.id}&select=*&order=made_at.desc&limit=500`,
      { headers: authHeaders }
    );

    if (!resp.ok) {
      const txt = await resp.text().catch(() => '');
      console.warn(`[syncPicksFromServer] HTTP ${resp.status}`, txt.slice(0, 200));
