...
async function ensureProfile(){
  try{
    ...
    // Use REST API — but wrap in its own try so a 400 here doesn't cascade
    // to marking Supabase offline (profiles table may not support upsert)
    const r = await fetch(`${SUPA_REST}/profiles`, {
      method:'POST',
    ...
    if(r.ok) markSupaOk();
    // If 400/409, just log — don't mark Supabase as offline
    else if(r.status >= 500) throw new Error(`HTTP ${r.status}`);
    else console.warn('ensureProfile: HTTP', r.status, '— table may not support client upsert');
...
async function renderLeaderboardPro(){
  ...
  const rowHTML = (r, i)=>{
    const nm = (r?.name && String(r.name).trim()) ? String(r.name).trim() : _getDisplayNameForUserId(r.id);
        ...
        <td style="white-space:nowrap;">
          <div style="display:flex;align-items:center;gap:10px">
            ...
            <div style="min-width:0">
              ...
              <div style="color:var(--dim);font-size:12px;margin-top:2px">${_pill(`${_tierIcon(tier)} ${tier}`)} ${r.topSport? _pill(r.topSport):''}</div>
              ${prov}
            </div>
          </div>
        </td>
