...
async function ensureProfile(){
  try{
    ...
    // Use REST API (sbUpsert) instead of supabase-js client which isn't loaded
    await sbUpsert('profiles', { user_id: currentUser.id, display_name: displayName });
...
async function publishToLeaderboard(){
  try{
    ...
    const w = settled.filter(p=>normalizeResult(p.result)==='won').length;
    const l = settled.filter(p=>normalizeResult(p.result)==='lost').length;
    const p = settled.filter(p=>normalizeResult(p.result)==='push').length;
    const total = w + l + p;

    // Calculate bankroll and PnL
    let bankroll = typeof STARTING_BANKROLL !== 'undefined' ? STARTING_BANKROLL : 1000;
    let pnl = 0;
    settled.forEach(pick => {
      if(pick.wager){
        if(normalizeResult(pick.result)==='won'){
          const profit = typeof calcPayout === 'function' ? calcPayout(pick.wager, pick.odds||-110) : pick.wager;
          bankroll += profit;
          pnl += profit;
        } else if(normalizeResult(pick.result)==='lost'){
          bankroll -= pick.wager;
          pnl -= pick.wager;
        }
      }
    ...
    // Get Sharp Rating if computed
    let singles90 = null;
    try{
      const snap = typeof computeRatingsDaily === 'function' ? computeRatingsDaily() : null;
      if(snap?.overall90?.rating != null) singles90 = JSON.stringify(snap.overall90);
    }catch{}
    ...
    if(singles90) row.singles_overall_90 = singles90;

    await sbUpsert('user_ratings', row);
  }catch(e){
    // Silent fail — leaderboard sync is non-critical
    console.warn('publishToLeaderboard failed:', e?.message || e);
  }
}
...
async function fetchLeaderboard(){
  try{
    ...
    // NOTE: Your Supabase `user_ratings` schema does NOT include the old
    // columns like sharp_rating_90 / win_rate_90 / roi_90 / units_90.
    // So we query only columns that exist (per your schema screenshot),
    // then compute what the UI needs locally.

    let rows = null;

    const selectCols =
      'user_id,name,w,l,p,total,bankroll,pnl,recent_picks,singles_overall_90,parlays_90,singles_overall_all_time,parlays_all_time';

    // Order by the closest “sharp rating” you actually have: singles_overall_90
    const orderCol = 'singles_overall_90';
    ...
    if(typeof window !== 'undefined' && window.supabase && window.supabase?.from){
      ...
      try{
        rows = await sbSelect('user_ratings', `select=${encodeURIComponent(selectCols)}&order=${orderCol}.desc&limit=${limit}`);
      }catch(e1){
        console.warn('Leaderboard query failed, retrying without order:', e1?.message||e1);
        rows = await sbSelect('user_ratings', `select=${encodeURIComponent(selectCols)}&limit=${limit}`);
        try{ rows = (rows||[]).slice().sort((a,b)=>_num(b[orderCol],0)-_num(a[orderCol],0)); }catch{}
      }
    }

    if(!rows || !rows.length) return [];

    // 2) Fetch display names in a second query (profiles.display_name wins)
    ...
    if(ids.length){
      try{
        if(typeof window !== 'undefined' && window.supabase && window.supabase?.from){
          const { data: profs, error: pErr } = await window.supabase
            .from('profiles')
    ...
    return rows.map(r=>{
      ...
      const l = _num(r.l, 0);
      const p = _num(r.p, 0);
      const total = _num(r.total, 0);

      // Compute “picks/pending/settled”
      // We don't have pending in user_ratings schema, so set pending=0 for now.
      // (If you add pending later, this will be easy to wire.)
      const picks = total || (w + l + p);
      const pending = 0;
      const settled = picks;

      // Compute win% (pushes excluded from denom)
      const denom = Math.max(0, (w + l));
      const winRate = denom ? (w / denom) * 100 : 0;

      // Compute units/ROI: we only have pnl + bankroll in schema.
      const bankroll = _num(r.bankroll, 0);
      const pnl = _num(r.pnl, 0);
      const units = pnl; // treat pnl as “units” for now
      const roi = bankroll ? (pnl / bankroll) * 100 : 0;

      // “Sharp rating”: use singles_overall_90 if present, else fall back to winRate
      let sharp = _num(r.singles_overall_90, NaN);
      if(Number.isNaN(sharp)) sharp = winRate;
      ...
      return {
        ...
        tier: (typeof _tierFromRating === 'function')
          ? _tierFromRating(sharp, picks)
          : (typeof _tier === 'function' ? _tier(sharp, picks) : '—'),
