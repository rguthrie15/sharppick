...
// History view state (prevents ReferenceErrors that blank the page)
let histSearch = '';
...
// Prop builder pending selection (so props behave like regular games: select -> wager -> place)
let pendingPropSel = null;
      // WebSocket (reserved)
...
function setMode(m){
  ...
  const dBtns={scores:'btnScores',calendar:'btnCalendar',history:'btnHistory',
    leaderboard:'btnLeaderboard',news:'btnNews',analysis:'btnAnalysis',contests:'btnContests',picks:'btnPicks',bracket:'btnBracket',trends:'btnTrends',market:'btnMarket',feed:'btnFeed',battles:'btnBattles'};
  ...
  const moreViews=['history','trends','market','battles','analysis','contests','bracket','calendar','news','feed'];
  ...
  const viewMap={scores:'scoresView',calendar:'calendarView',history:'historyView',
    leaderboard:'leaderboardView',news:'newsView',analysis:'analysisView',contests:'contestsView',bracket:'bracketView',trends:'trendsView',market:'trendsView',feed:'feedView',battles:'battlesView',myaction:'myactionView'};
  ...
  if(m==='trends' || m==='market') renderTrendsDashboard();
...
async function fetchLeaderboard(){
  try{
    ...
    // This app primarily uses the Supabase REST endpoint (SUPA_REST + SUPA_HDR).
    // Some older builds had supabase-js available on window; if it exists, we can use it,
    // but we never require it for leaderboard to work.
    let rows = null;

    if(typeof window !== 'undefined' && window.supabase && window.supabase?.from){
      const { data, error } = await window.supabase
        .from('user_ratings')
        .select('user_id,sharp_rating_90,win_rate_90,roi_90,units_90,picks_90,pending_90,all_time_singles,top_sport,top_sport_rating,cur_streak,avg_odds_last10,is_provisional,provisional_reason,profiles!user_ratings_user_id_fkey(display_name)')
        .order('sharp_rating_90', { ascending: false, nullsFirst: false })
        .limit(limit);

      if(error) throw error;
      rows = data;
    }else{
      // REST (PostgREST) path — works in all deployments
      // NOTE: PostgREST relationship embeds can 400 if the FK/relationship name differs across environments.
      // We fetch ratings first, then fetch display_names from profiles in a second call.
      rows = await sbSelect(
        'user_ratings',
        `select=user_id,sharp_rating_90,win_rate_90,roi_90,units_90,picks_90,pending_90,all_time_singles,top_sport,top_sport_rating,cur_streak,avg_odds_last10,is_provisional,provisional_reason&order=sharp_rating_90.desc&limit=${limit}`
      );
      ...
      try{
        ...
        if(ids.length){
          ...
          const profs = await sbSelect('profiles', `select=user_id,display_name&user_id=in.(${idList})`);
          const pmap = {};
          (profs||[]).forEach(p=>{ if(p?.user_id) pmap[p.user_id]=p.display_name; });
          rows = (rows||[]).map(r=>({ ...r, profiles: { display_name: pmap[r.user_id] } }));
        }
      }catch(e){ /* non-fatal */ }
    }
    ...
    return rows.map(r=>{
      const sharp = _num(r.sharp_rating_90, 0);

      // Name (critical): prefer profiles.display_name, then any legacy field, then fallback
      const profileName =
        (r.profiles && (r.profiles.display_name || r.profiles.username || r.profiles.name)) ||
        r.display_name || r.name || '';

      const name = (String(profileName || '').trim()) || (`user-${String(r.user_id||'').slice(0,6)}`);

      const picks = _num(r.picks_90, 0);
      const pending = _num(r.pending_90, 0);
      const settled = Math.max(0, picks - pending);
      ...
      return {
        ...
        sharp,
        tier: _tier(sharp, picks),

        winRate: _num(r.win_rate_90, 0),
        roi: _num(r.roi_90, 0),
        units: _num(r.units_90, 0),
...
async function fetchSharpLeaderboard(tab){
  ...
  // Fallback 1: user_ratings table (server-calculated; no proprietary math in frontend)
  try{
    // Columns expected: user_id, singles_overall_90, singles_picks_90, parlays_90, etc.
    const orderCol = 'singles_overall_90.desc.nullslast';
    const rows = await sbSelect('user_ratings', `select=*&order=${orderCol}&limit=${limit_n}`);
    return rows || [];
  }catch(e){}

  // Fallback 2: empty (UI will show guidance)
  return [];
}
      ...
      <div class="prop-browse-grid">
        ${players.map(prop=>{
          const existingPick = picks.find(pk=>pk.gameId===prop.id&&pk.result==='pending');
           const pending = (pendingPropSel && pendingPropSel.propId===prop.id) ? pendingPropSel : null;
          return `<div class="prop-browse-card">
            ...
            <div class="prop-browse-btns">
              <div class="prop-browse-btn ${((existingPick||pending)?.side==='over')?'picked':''}"
              ...
              <div class="prop-browse-btn ${((existingPick||pending)?.side==='under')?'picked':''}"
...
function parseAmericanOddsStr(s, fallback=-110){
  try{
    if(s==null) return fallback;
    let t=String(s).trim();
    t=t.replace(/−/g,'-'); // normalize unicode minus
...
function makePropBuilderPick(propId, player, stat, line, side, gameId){
  // NEW behavior: select -> choose wager -> place (same flow as regular games)
  if(!currentUser){ alert('Enter your name first.'); return; }

  const game = allGames.find(g=>g.id===gameId)||{};
  const prop = (propBuilderProps||[]).find(p=>p.id===propId) || {};
  const odds = side==='over' ? parseAmericanOddsStr(prop.overOdds, -110) : parseAmericanOddsStr(prop.underOdds, -110);

  pendingPropSel = {
    propId, player, stat, line, side, gameId,
    odds,
    wager: DEFAULT_WAGER,
  ...
  // Re-render to show selection state + footer
  const existGame = allGames.find(g=>g.id===gameId);
  if(existGame) fetchAndRenderProps(gameId, existGame);
  renderPropBuilderFooter();
}

function clearPendingPropSel(){
  pendingPropSel = null;
  const foot = document.getElementById('propBuilderFooter');
  if(foot) foot.innerHTML = '';
  // Also refresh card highlight state if prop builder is open
  try{
    const gid = propBuilderGameId;
    const g = allGames.find(x=>x.id===gid);
    if(g) fetchAndRenderProps(gid, g);
  }catch{}
}

function updatePendingPropWager(amount){
  if(!pendingPropSel) return;
  const bankData = computeBankroll();
  const bal = typeof bankData === 'object' ? (bankData.balance||STARTING_BANKROLL) : (bankData||STARTING_BANKROLL);
  const maxBet = Math.floor(bal);
  pendingPropSel.wager = Math.max(1, Math.min(Math.round(amount), maxBet||STARTING_BANKROLL));
  renderPropBuilderFooter();
}

function placePendingPropPick(){
  if(!pendingPropSel) return;
  const sel = pendingPropSel;
  const desc = `${sel.player} ${sel.side==='over'?'O':'U'} ${sel.line} ${sel.stat}`;

  // Remove any existing pending pick for this same prop
  picks = picks.filter(p=>!(p.gameId===sel.propId && p.result==='pending'));

  picks.push({
    gameId: sel.propId,
    actualGameId: sel.gameId,
    type: 'prop',
    side: sel.side,
    description: desc,
    gameStr: sel.gameStr,
  ...
  savePicks();
  updateBankrollUI();
  showWinToast(`🎯 Pick placed: ${desc}`);

  // Clear selection + re-render cards
  const gid = sel.gameId;
  pendingPropSel = null;
  const g = allGames.find(x=>x.id===gid);
  if(g) fetchAndRenderProps(gid, g);
  renderPropBuilderFooter();
...
function renderPropBuilderFooter(){
  const foot = document.getElementById('propBuilderFooter');
  if(!foot) return;
  if(!pendingPropSel){ foot.innerHTML=''; return; }
  ...
  const profit = calcPayout(pendingPropSel.wager, pendingPropSel.odds);

  foot.innerHTML = `
    <div class="prepick-panel" style="margin-top:12px">
      <div style="display:flex;justify-content:space-between;align-items:center">
        <div style="font-family:'DM Mono',monospace;font-size:10px;letter-spacing:1px;color:var(--muted)">WAGER</div>
        <button class="parlay-add-btn" style="width:auto;margin:0" onclick="clearPendingPropSel()">CANCEL</button>
      </div>
      <div class="wager-row" style="margin-top:8px">
        <div class="wager-presets">
          ${presets.map(amt =>
            `<div class="wager-preset ${pendingPropSel.wager===amt?'active':''}" onclick="updatePendingPropWager(${amt})">$${amt}</div>`
          ).join('')}
        </div>
        <input class="wager-custom" type="number" min="1" max="${maxBet}" value="${pendingPropSel.wager}"
          onchange="updatePendingPropWager(+this.value)" onclick="event.stopPropagation()">
      </div>
      <div class="wager-payout">Win: <span class="win-amt">+$${profit}</span> &middot; Risk: $${pendingPropSel.wager} &middot; Odds: ${pendingPropSel.odds}</div>
      <div style="display:flex;gap:6px;margin-top:8px">
        <button class="place-pick-btn" style="flex:2" onclick="placePendingPropPick()">&#10003;  PLACE PICK</button>
      </div>
    </div>`;
}
...
function startPicksRealtime() {
    ...
    picksRealtimeSub = supabase
      ...
      .on(
        ...
        async (payload) => {
          ...
          // If this fires during init (before currentUser is set), queue a sync
          if(!currentUser?.id){ _pendingPickSync = true; return; }
  ...
  if (status === 'SUBSCRIBED') {
    ...
    // Initial pull (fixes the "second tab needs refresh" issue)
    try{ syncPicksFromServer(false); }catch{}
  }
});
  } catch (e) {
