...
async function fetchLeaderboard(){
  try{
    ...
    // 1) Fetch ratings without relationship embeds (embeds caused 400s in some PostgREST setups)
    let rows = null;
    ...
    if(typeof window !== 'undefined' && window.supabase && window.supabase?.from){
      ...
      console.warn('Leaderboard primary query failed, retrying without order:', e1?.message||e1);
      rows = await sbSelect(
        'user_ratings',
        `select=user_id,sharp_rating_90,win_rate_90,roi_90,units_90,picks_90,pending_90,all_time_singles,top_sport,top_sport_rating,cur_streak,avg_odds_last10,is_provisional,provisional_reason,recent_picks,bankroll&limit=${limit}`
      );
      try{ rows = (rows||[]).slice().sort((a,b)=>_num(b.sharp_rating_90,0)-_num(a.sharp_rating_90,0)); }catch{}
    }
    }

    if(!rows || !rows.length) return [];

    // 2) Fetch display names in a second query
    ...
    return rows.map(r=>{
      ...
      const name =
        ...
        (String(r.display_name||r.name||'').trim()) ||
      ...
      return {
        ...
        sharp,
        tier: _tierFromRating ? _tierFromRating(sharp, picks) : (typeof _tier==='function' ? _tier(sharp, picks) : '—'),
