...
async async function ensureProfile(){
  try{
...
function _getDisplayNameForUserId(uid){
  // Prefer current user name
  try{
    if(currentUser?.id && uid === currentUser.id && currentUser?.name) return currentUser.name;
  }catch{}
...
function _getLocalRecord(){
  try{
    ...
    ps.forEach(x=>{
      ...
      if(r==='won' || r==='win') w++;
      else if(r==='lost' || r==='loss') l++;
      else if(r==='push' || r==='p') p++;
      else pending++;
    });
    return {w,l,p,pending, total: w+l+p};
  }catch{
    return {w:0,l:0,p:0,pending:0,total:0};
  }
}
...
async function fetchLeaderboard(){
  try{
    const limit = 250;
    // Pull server-calculated ratings (keeps formula private)
    ...
    return rows.map(r=>{
      ...
      try{
        if(currentUser?.id && r.user_id === currentUser.id){
          ...
          // also reflect pending instantly for current user
          if(typeof lr.pending === 'number'){
            r.pending_90 = Math.max(_num(r.pending_90,0), lr.pending);
          }
        }
      }catch{}
...
// ─────────────────────────────────────────────────────────────
// Realtime listener for leaderboard (refresh UI when user_ratings changes)
// ─────────────────────────────────────────────────────────────
let leaderboardRealtimeSub = null;
let _lbRefreshT = null;
...
function startLeaderboardRealtime(){
  try{
    ...
    leaderboardRealtimeSub = supabase
      .channel('leaderboard_user_ratings')
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'user_ratings' },
    ...
    console.warn('Leaderboard realtime error:', e?.message || e);
  }
}

function stopLeaderboardRealtime(){
  try{ leaderboardRealtimeSub?.unsubscribe?.(); }catch{}
  leaderboardRealtimeSub = null;
}

function scheduleLeaderboardRefresh(){
  try{ clearTimeout(_lbRefreshT); }catch{}
  _lbRefreshT = setTimeout(() => {
    try{
      if(typeof MODE !== 'undefined' && MODE === 'board'){
...
if (restored && currentUser) {
  ...
  startPicksRealtime();
  startLeaderboardRealtime();   // ADD THIS
